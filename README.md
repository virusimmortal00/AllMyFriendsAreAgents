...
Conversation energy has four levels: **Low** usually yields one respondent, **Balanced** usually one or two, **Lively** permits several participants, and **Party** scales participation to the configured roster while retaining an emergency ceiling. When agents explicitly mark a discussion unresolved, the server runs a bounded synthesis, objection, and reconciliation phase. If that phase still needs human input or reaches its safety limit, the transcript says why and **Actions → Continue discussion** starts another bounded round.
