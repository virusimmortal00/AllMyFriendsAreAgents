...
const COMMAND_CATALOG: Readonly<Record<RoomCommandName, RoomCommandCatalogEntry>> = {
  help: {
    command: "help",
    summary: "List the room commands currently available to you.",
    syntax: "/help",
    example: "/help",
  },
  task: {
    command: "task",
    summary: "Delegate bounded work to one eligible agent.",
    syntax: "/task [@agent] <bounded work>",
    example: "/task @Sol check the error path",
  },
  pov: {
    command: "pov",
    summary: "Request bounded perspectives from all eligible agents or one named agent.",
    syntax: "/pov [@agent] <question>",
    example: "/pov @Sol What tradeoff are we missing?",
  },
  poll: {
    command: "poll",
    summary: "Create a server-authoritative poll with quoted options.",
    syntax: '/poll "Question" "Option A" "Option B"',
    example: '/poll "Ship today?" "Yes" "No"',
  },
  gh: {
    command: "gh",
    summary: "Read bounded context from this room's verified project repository.",
    syntax: "/gh recent | pr <number> | issue <number> | ci [<number>]",
    example: "/gh pr 98",
  },
};
...
export function commandHelpText(commands: readonly RoomCommandName[]) {
  ...
  return [
    "Room commands available to you:",
    ...entries.map(
...
function structuredExample(command: RoomCommandName) {
  ...
  if (command === "task")
    return '{"command":"task","prompt":"Check the error path","selection":{"kind":"pinned","agentId":"codex-sol"}}';
  if (command === "pov")
    return '{"command":"pov","prompt":"What tradeoff are we missing?","selection":{"kind":"all-eligible"}} or {"command":"pov","prompt":"What tradeoff are we missing?","selection":{"kind":"pinned","agentId":"codex-sol"}}';
  if (command === "gh")
    return '{"command":"gh","selector":{"kind":"recent"}} or {"command":"gh","selector":{"kind":"pr","number":98}}';
...
export type CommandInvocation =
  | {
      ...
      readonly selection:
        ...
        | { readonly kind: "pinned"; readonly agentId: ActiveAgentId };
    }
  | {
      readonly command: "pov";
      readonly prompt: string;
      readonly selection:
...
export function parseCommand(text: string): CommandParseResult {
  ...
  if (!ROOM_COMMANDS.includes(command as RoomCommandName))
    return { kind: "private-error", message: `Unknown command /${command}. Try /help.` };
  if (command === "help")
    return rest
      ? { kind: "private-error", message: "/help does not take any arguments." }
      : { kind: "command", invocation: { command: "help" } };
  ...
  if (rest.startsWith("@")) {
    ...
    const agentId = pinned?.[1];
    if (!agentId)
      return { kind: "private-error", message: "Choose a roster agent after /task, or omit @agent for round-robin." };
    const prompt = pinned[2]?.trim() ?? "";
    if (prompt.length > MAX_PROMPT_LENGTH)
      return { kind: "private-error", message: `Keep /task prompts under ${MAX_PROMPT_LENGTH} characters.` };
    return { kind: "command", invocation: { command: "task", prompt, selection: { kind: "pinned", agentId } } };
  }
  if (rest.length > MAX_PROMPT_LENGTH)
    return { kind: "private-error", message: `Keep /task prompts under ${MAX_PROMPT_LENGTH} characters.` };
...
export function parseCommandInput(input: CommandInput): CommandParseResult {
  ...
  if (!input || typeof input !== "object")
    return { kind: "private-error", message: "Try /help to see the available commands." };
  ...
  if (input.command === "gh" && input.selector && typeof input.selector === "object") {
    ...
    if (
      (selector.kind === "pr" || selector.kind === "issue") &&
      keys === "kind,number" &&
      typeof selector.number === "number"
    )
      return parseCommand(`/gh ${selector.kind} ${selector.number}`);
    if (
      selector.kind === "ci" &&
      (keys === "kind" || keys === "kind,number") &&
      (selector.number === undefined || typeof selector.number === "number")
    )
      return parseCommand(`/gh ci${selector.number === undefined ? "" : ` ${selector.number}`}`);
    return ghError();
  }
  if (
    input.command === "pov" &&
    ...
    (input.selection === undefined ||
      input.selection?.kind === "all-eligible" ||
      (input.selection?.kind === "pinned" && typeof input.selection.agentId === "string"))
  ) {
    const prompt = input.prompt.trim();
    if (!prompt) return { kind: "private-error", message: "Add a prompt after /pov." };
    if (prompt.length > MAX_PROMPT_LENGTH)
      return { kind: "private-error", message: `Keep /pov prompts under ${MAX_PROMPT_LENGTH} characters.` };
    return {
      kind: "command",
      invocation: { command: "pov", prompt, selection: input.selection || { kind: "all-eligible" } },
    };
  }
  if (
    input.command === "task" &&
    typeof input.prompt === "string" &&
    (input.selection?.kind === "round-robin" ||
      (input.selection?.kind === "pinned" && typeof input.selection.agentId === "string"))
  ) {
    const prompt = input.prompt.trim();
    if (prompt.length > MAX_PROMPT_LENGTH)
  ...
  if (input.command === "poll" && typeof input.question === "string" && Array.isArray(input.options)) {
    ...
    if (input.options.every((option) => typeof option === "string"))
      return parseCommand(`/poll ${[input.question, ...input.options].map(quote).join(" ")}`);
  }
...
function parseGh(rest: string): CommandParseResult {
  ...
  if (parts.length === 1 && parts[0]?.toLocaleLowerCase() === "recent")
    return { kind: "command", invocation: { command: "gh", selector: { kind: "recent" } } };
  const selector = parts[0]?.toLocaleLowerCase();
  if (selector === "ci" && parts.length === 1)
    return { kind: "command", invocation: { command: "gh", selector: { kind: "ci" } } };
  if ((selector === "pr" || selector === "issue" || selector === "ci") && parts.length === 2) {
    const number = canonicalPositiveDecimal(parts[1]!);
    if (number !== undefined)
      return { kind: "command", invocation: { command: "gh", selector: { kind: selector, number } as GhSelector } };
  }
  return ghError();
}
...
function ghError(): CommandParseResult {
  return {
    kind: "private-error",
    message:
      "Use /gh recent, /gh pr <number>, /gh issue <number>, or /gh ci [<number>] with a positive decimal number.",
  };
}
...
function parsePov(rest: string): CommandParseResult {
  ...
  if (rest.startsWith("@")) {
    ...
    const prompt = pinned?.[2]?.trim() ?? "";
    if (!pinned || !prompt)
      return { kind: "private-error", message: "Choose one roster agent and add a prompt after /pov @agent." };
    if (prompt.length > MAX_PROMPT_LENGTH)
      return { kind: "private-error", message: `Keep /pov prompts under ${MAX_PROMPT_LENGTH} characters.` };
    return {
      kind: "command",
      invocation: { command: "pov", prompt, selection: { kind: "pinned", agentId: pinned[1]! } },
    };
  }
  if (rest.length > MAX_PROMPT_LENGTH)
    return { kind: "private-error", message: `Keep /pov prompts under ${MAX_PROMPT_LENGTH} characters.` };
...
function parsePoll(rest: string): CommandParseResult {
  ...
  return {
    kind: "command",
    invocation: { command: "poll", question: question!, options: [first!, second!, ...remaining] },
  };
}
...
function pollError(): CommandParseResult {
  return {
    kind: "private-error",
    message: 'Use /poll "Question" "Option A" "Option B" with one question and at least two quoted options.',
  };
}
...
export const ALLOW_ALL_COMMANDS: CommandPermissions = {
  allowAll: true,
...
export function validCommandPermissions(input: unknown): input is CommandPermissions {
  ...
  if (
    (value as { catalogRevision?: unknown }).catalogRevision !== undefined &&
    (value as { catalogRevision?: unknown }).catalogRevision !== COMMAND_CATALOG_REVISION
  )
    return false;
  ...
  return (
    value.allowed.every(
      (command) => typeof command === "string" && ROOM_COMMANDS.includes(command as RoomCommandName),
...
export function normalizeCommandPermissions(input: unknown): CommandPermissions {
  ...
  if (value.allowAll) {
    const legacyAllowed = Array.isArray(value.allowed)
      ? value.allowed.filter(
          (candidate): candidate is RoomCommandName =>
            typeof candidate === "string" &&
            LEGACY_ROOM_COMMANDS.includes(candidate as (typeof LEGACY_ROOM_COMMANDS)[number]),
        )
      : [...LEGACY_ROOM_COMMANDS];
  ...
  const allowed = [
    ...new Set(
      value.allowed.filter(
        (candidate): candidate is RoomCommandName =>
          typeof candidate === "string" && ROOM_COMMANDS.includes(candidate as RoomCommandName),
      ),
    ),
  ].sort((left, right) => ROOM_COMMANDS.indexOf(left) - ROOM_COMMANDS.indexOf(right));
  return {
    allowAll: false,
    allowed,
    ...(value.catalogRevision === COMMAND_CATALOG_REVISION ? { catalogRevision: COMMAND_CATALOG_REVISION } : {}),
  };
}
...
export function effectiveAllowedCommands(agent: CommandPermissions, ceiling: readonly RoomCommandName[]) {
  ...
  const requested =
    agent.allowAll && agent.catalogRevision === COMMAND_CATALOG_REVISION ? ROOM_COMMANDS : agent.allowed;
...
export type RoundRobinResolution =
  | {
      ...
      readonly agentId: ActiveAgentId;
      readonly nextLastAssignedAgentId: ActiveAgentId | null;
      readonly advancePointer: boolean;
    }
  | {
      readonly kind: "no-eligible-candidates";
      readonly nextLastAssignedAgentId: ActiveAgentId | null;
      readonly advancePointer: false;
    };

export function resolveRoundRobin(
  candidates: readonly SelectionCandidate[],
  lastAssignedAgentId: ActiveAgentId | null,
  ...
  if (pinnedAgentId) {
    ...
    return pinned?.eligible
      ? {
          ...
          nextLastAssignedAgentId: lastAssignedAgentId,
  ...
  for (let offset = 0; offset < candidates.length; offset++) {
    ...
    if (candidate.eligible)
      return {
        ...
        nextLastAssignedAgentId: candidate.agentId,
        advancePointer: true,
      };
  }
  return { kind: "no-eligible-candidates", nextLastAssignedAgentId: lastAssignedAgentId, advancePointer: false };
