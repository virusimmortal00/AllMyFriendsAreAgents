...
const PARTICIPANT_STYLE_DEFAULTS = {
  you: {
    fontFamily: "Arial",
    fontSize: 17,
    textColor: "#1618fd",
...
} satisfies ParticipantStyles;

export const DEFAULT_PARTICIPANT_STYLES: ParticipantStyles & typeof PARTICIPANT_STYLE_DEFAULTS =
  PARTICIPANT_STYLE_DEFAULTS;
...
export function sanitizeChatStyle(input: unknown, fallback: ChatStyle): ChatStyle {
  ...
  return {
    fontFamily: CHAT_FONT_FAMILIES.includes(value.fontFamily as ChatFontFamily)
      ? (value.fontFamily as ChatFontFamily)
      : fallback.fontFamily,
    fontSize:
...
export function normalizeParticipantStyles(input: unknown): ParticipantStyles {
  ...
  for (const participant of participants) {
    const legacy =
      participant === "codex-sol" ? value.codex : participant === "claude-sonnet" ? value.claude : undefined;
    const fallback = DEFAULT_PARTICIPANT_STYLES[participant] ?? DEFAULT_PARTICIPANT_STYLES["codex-sol"];
    normalized[participant] = sanitizeChatStyle(value[participant] ?? legacy, fallback);
  }
  return normalized;
}
...
export function extractStyleDirective(text: string, fallback: ChatStyle): ChatStyle | undefined {
  ...
  try {
    return sanitizeChatStyle(JSON.parse(payload), fallback);
  } catch {
    return undefined;
  }
}
