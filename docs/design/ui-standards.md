...
| UI-ADMIN-001 | Surfaces that need server-administrator authority are pages of one Server Administration window opened from the Server menu. The window owns sign-in: administrator pages and their Server menu commands are disabled until Owner login succeeds, and pages never render their own sign-in controls. Outside the window, blocked actions use the one `AdministratorRequired` notice. Member-visible room data (such as the room's repository) stays outside administration. | `AdministrationWindow` hosts Owner login, Integrations, Rooms & repositories, and Diagnostics through `ExplorerLayout` inside `DialogFrame` and disables pages marked `requiresAdministrator` while signed out; `App.tsx` disables the matching Server menu commands. | `src/layout-structure.test.tsx` requires administration pages to use `administration-page`/`page-header` inside `ExplorerLayout`; `src/reconnect-flow.test.tsx` proves those commands and pages are disabled while signed out and enabled after Owner login. |
