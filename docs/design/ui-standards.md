...
| UI-MENU-002 | There is no workspace switcher. Commands that open another surface live in the menu that owns it (You, Room, Server) and end with an ellipsis; View never opens anything. | `defineViewMenu` accepts only `presentationCommand` values; the Server menu opens `AdministrationWindow` at a page. | `src/ui-standards.test.tsx` proves non-presentation commands fail closed in `View` and that `App.tsx` has no workspace-switching surface; `src/reconnect-flow.test.tsx` proves the menu bar is You · Room · Server · View · Help. |
| UI-EXIT-001 | Chat is never replaced. Larger tools open as top-level windows over the chat window, with a title-bar close, a Close action, Escape, and focus returned to the opener. | Top-level windows are `DialogFrame` instances; `App.tsx` renders the chat window unconditionally. | `src/reconnect-flow.test.tsx` opens Server Administration from the Server menu, confirms the transcript stays mounted, closes it, and checks focus restoration; visual scenarios click the administration close control. |
...
| UI-STRUCT-001 | Application windows stay centered and screen-efficient; large tool windows use most of the screen and phones give them the full viewport; long dialogs separate a scrolling body from persistent actions. | `.desktop`/`.app-window`, `WorkspaceSurface`, `workspace-view`, and `DialogFrame` own those structures. | `src/mobile-layout.test.ts`, `src/layout-structure.test.tsx`, and rendered browser checks for changed layouts. |
| UI-ADMIN-001 | Surfaces that need server-administrator authority are pages of one Server Administration window opened from the Server menu. The window owns sign-in: Server menu commands for administrator pages are disabled until Owner login succeeds, and inside the window those pages open as previews with every control disabled, and pages never render their own sign-in controls. Outside the window, blocked actions use the one `AdministratorRequired` notice. Member-visible room data (such as the room's repository) stays outside administration. | `AdministrationWindow` hosts Owner login, Integrations, Rooms & repositories, and Diagnostics through `ExplorerLayout` inside `DialogFrame` and wraps pages marked `requiresAdministrator` in a disabled preview fieldset while signed out; `App.tsx` disables the matching Server menu commands. | `src/layout-structure.test.tsx` requires administration pages to use `administration-page`/`page-header` inside `ExplorerLayout`; `src/reconnect-flow.test.tsx` proves the commands are disabled while signed out, page previews have disabled controls, and pages work after Owner login. |
...
- **View** changes how the chat window is presented. It never opens another surface.
- **Help** contains documentation and support entry points.

New `View` commands must be created with `presentationCommand`. There are no full-workspace destinations: a larger tool opens as a top-level window built on `DialogFrame` from the menu that owns it. Product code must not use type assertions to bypass these builders.

## Windows 95 composition primitives

- **Explorer window** (`ExplorerLayout`): a fixed page list beside one inset content pane and a sunken status line. Use it inside a top-level window that has several related pages; do not add a second blue header band inside the window title strip. Page headings are plain 13px bold text over a groove separator.
