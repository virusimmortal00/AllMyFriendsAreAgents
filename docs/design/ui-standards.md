...
| UI-ADMIN-001 | Surfaces that need server-administrator authority are pages of one Server Administration window opened from the Server menu. The window owns sign-in: locked pages show the single Owner login form in place, and pages never render their own sign-in controls. Outside the window, blocked actions use the one `AdministratorRequired` notice. Member-visible room data (such as the room's repository) stays outside administration. | `AdministrationWindow` hosts Owner login, Integrations, Rooms & repositories, and Diagnostics through `ExplorerLayout` inside `DialogFrame` and gates pages marked `requiresAdministrator`. | `src/layout-structure.test.tsx` requires administration pages to use `administration-page`/`page-header` inside `ExplorerLayout`; `src/reconnect-flow.test.tsx` proves a locked page shows exactly one sign-in control and unlocks in place. |
