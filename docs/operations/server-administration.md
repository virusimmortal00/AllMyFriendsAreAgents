...
The **Server** menu opens the Server Administration window at one of its pages: **Owner login**, **Integrations** (GitHub and OpenRouter), **Rooms & repositories**, **Room behavior**, and **Diagnostics**. **Room behavior** contains the administrator-only base prompt, summarizer, and agent-routing controls. **Owner login** shows whether the server is unclaimed, claimed with no session, or signed in. Its username and role identify the durable control-plane principal independently of the editable room screen name. While signed out, the administrator pages open as disabled previews and their **Server** menu commands are unavailable. **Room Properties** contains only member-editable room name, topic, and conversation energy settings.
