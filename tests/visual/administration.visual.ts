...
test("administration recovery and room membership", async ({ page }) => {
  ...
  await expect(page.getByRole("tab", { name: "Agent behavior", exact: true })).toHaveCount(0);
  await page.getByRole("button", { name: "Close Room Properties" }).click();
  await menu("Server", "Owner login...");
  ...
  await page.getByRole("button", { name: "Sign in", exact: true }).click();
  await page.getByRole("tab", { name: "Room behavior", exact: true }).click();
  await page.getByLabel("Additional room prompt", { exact: true }).fill("Fictional owner update after sign-in.");
