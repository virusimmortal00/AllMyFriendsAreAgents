...
import { publicPollProjection, type CommandRecordStore } from "./command-record.js";
...
export function registerCommandRoutes(input:{app:express.Express;runtime:CommandRuntime;store:CommandRecordStore;humans:HumanPresenceRegistry;sessions:HumanSessions;developers:DeveloperTeamRegistry}) {
  ...
  const canReadDiagnostics=(authorization:string|undefined)=>developers.authenticate(authorization,"DIAGNOSTIC_READ")||developers.authenticate(authorization,"ROOM_READ");
  ...
  app.post("/api/polls/:pollId/votes",async(request,response)=>{const human=sessionHuman(request,humans,sessions);if(!human)return response.status(401).json({error:"Join the room before voting."});const result=await runtime.vote(String(request.params.pollId),human.id,request.body?.optionIndex);return response.status(result.kind==="accepted"?result.duplicate?200:201:400).set("Cache-Control","no-store").json(result);});
  app.get("/api/polls",async(request,response)=>{if(!sessionHuman(request,humans,sessions))return response.status(401).json({error:"Join the room before viewing polls."});const polls=await store.listCommandPolls(CANONICAL_ROOM_ID);const items=await Promise.all(polls.map(async(poll)=>publicPollProjection(poll,await store.listCommandVotes(CANONICAL_ROOM_ID,poll.pollId))));return response.set("Cache-Control","no-store").json({items});});
  app.get("/api/polls/:pollId",async(request,response)=>{if(!sessionHuman(request,humans,sessions))return response.status(401).json({error:"Join the room before viewing polls."});const poll=await store.getCommandPoll(CANONICAL_ROOM_ID,String(request.params.pollId));if(!poll)return response.status(404).json({error:"Poll not found."});return response.set("Cache-Control","no-store").json(publicPollProjection(poll,await store.listCommandVotes(CANONICAL_ROOM_ID,poll.pollId)));});
  app.get("/api/developer/diagnostics",async(request,response)=>{if(!canReadDiagnostics(request.header("authorization")))return response.status(404).json({error:"Not found."});const agentId=typeof request.query.agentId==="string"&&isActiveAgentId(request.query.agentId)?request.query.agentId:null;if(!agentId)return response.status(400).json({error:"A valid room agent scope is required."});const search=typeof request.query.search==="string"?request.query.search:undefined;if(search&&search.length>200)return response.status(400).json({error:"Diagnostic search is limited to 200 characters."});const reason=typeof request.query.reason==="string"?request.query.reason.slice(0,80):undefined;const limit=Number(request.query.limit||50);return response.set("Cache-Control","no-store").json({items:await store.listDiagnostics(CANONICAL_ROOM_ID,{agentId,search,reason,limit:Number.isFinite(limit)?limit:50})});});
