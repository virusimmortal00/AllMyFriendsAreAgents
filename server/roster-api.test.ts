...
async function fixture(options:{control?:boolean;capabilities?:boolean}={}) {
  ...
  const capabilityStatuses=options.capabilities?()=>({"codex-sol":{agentId:"codex-sol",policyRevision:1 as const,capabilities:{conversation:{configured:true,runtimeAvailable:true,effective:true,reason:"available" as const,guidance:"safe"},github_read:{configured:false,runtimeAvailable:false,effective:false,reason:"not_configured" as const,guidance:"Configure server-only read access.",contract:"read-only" as const},project_write:{configured:false,runtimeAvailable:false,effective:false,reason:"governed_worker_only" as const,guidance:"Use a worker."}},effectiveCommands:[],issuableCommands:[],commands:{}}}):undefined;
