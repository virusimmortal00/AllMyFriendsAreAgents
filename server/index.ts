...
async function performTurnUnchecked({ agent, instruction, includeDiff = false, visibleMessageLimit = 3, preflight, deliveryId }: ConversationTurn) {
  ...
  const parsed = result.structuredTurn
    ? interpretStructuredRoomTurn(agent, result.structuredTurn, currentStyle, visibleMessageLimit, currentEnabledAgents())
    : parseAgentTurn(agent, result.text, currentStyle, visibleMessageLimit, currentEnabledAgents());
