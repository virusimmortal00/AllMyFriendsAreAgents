...
async function fixture(options:{clock?:FakeClock; roster?:()=>RoomAgentRoster; execute?:(agent:ActiveAgentId,prompt:string,hooks:CommandLaunchHooks)=>Promise<CommandExecutionResult>; deliver?:(attemptId:string,agent:ActiveAgentId,messages:readonly string[],result:CommandExecutionResult)=>Promise<void>; povExecute?:(agents:readonly ActiveAgentId[],prompt:string,signal:AbortSignal)=>Promise<void>; eligible?:(agent:ActiveAgentId)=>boolean;capacityOne?:boolean}={}){
  ...
  let reserved=false;const published=new Set<string>();const runtime=new CommandRuntime({store,clock,stage1Ms:10,stage2Ms:20,roster:options.roster||(()=>roster()),canLaunch:options.eligible||(()=>!reserved),...(options.capacityOne?{reserveLaunch:()=>{if(reserved)return undefined;reserved=true;return{release(){reserved=false;}};}}:{}),executeTask:options.execute|| (async(agent,_prompt,hooks)=>{await hooks.active(`generation-${agent}`);return{generationId:`generation-${agent}`,visibleMessages:[`reply-${agent}`]};}),executePov:options.povExecute|| (async(agents)=>{pov.push([...agents]);}),publishStatus:async(id,text)=>{if(!published.has(id)){published.add(id);statuses.push(text);}},deliverTask:options.deliver|| (async(_attemptId,agent,messages)=>{deliveries.push({agent,messages});})});
