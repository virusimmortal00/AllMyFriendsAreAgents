...
describe("durable command runtime",()=>{
  ...
  it("starts durable task and POV work even when audit publication fails",async()=>{let taskRuns=0;const task=await fixture({publish:async()=>{throw new Error("audit unavailable");},execute:async(agent,_prompt,hooks)=>{taskRuns++;await hooks.active("audit-task");return{generationId:"audit-task",visibleMessages:[agent]};}});await expect(task.runtime.submit("/task keep going",{kind:"human",id:"h1",displayName:"Ada"},"audit-failure-task-01")).rejects.toThrow("audit unavailable");await eventually(()=>expect(task.deliveries).toHaveLength(1));await expect(task.runtime.submit("/task keep going",{kind:"human",id:"h1",displayName:"Ada"},"audit-failure-task-01")).rejects.toThrow("audit unavailable");expect(taskRuns).toBe(1);
    let povRuns=0;const pov=await fixture({publish:async()=>{throw new Error("audit unavailable");},povExecute:async()=>{povRuns++;}});await expect(pov.runtime.submit("/pov keep going",{kind:"human",id:"h1",displayName:"Ada"},"audit-failure-pov-01")).rejects.toThrow("audit unavailable");await eventually(()=>expect(povRuns).toBe(1));});
