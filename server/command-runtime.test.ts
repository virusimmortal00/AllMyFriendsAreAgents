...
async function fixture(options:{clock?:FakeClock; roster?:()=>RoomAgentRoster; execute?:(agent:ActiveAgentId,prompt:string,hooks:CommandLaunchHooks)=>Promise<CommandExecutionResult>; eligible?:(agent:ActiveAgentId)=>boolean}={}){
  ...
  const runtime=new CommandRuntime({store,clock,stage1Ms:10,stage2Ms:20,roster:options.roster||(()=>roster()),canLaunch:options.eligible||(()=>true),executeTask:options.execute|| (async(agent,_prompt,hooks)=>{await hooks.active(`generation-${agent}`);return{generationId:`generation-${agent}`,visibleMessages:[`reply-${agent}`]};}),executePov:async(agents)=>{pov.push([...agents]);},publishStatus:async(text)=>{statuses.push(text);},deliverTask:async(agent,messages)=>{deliveries.push({agent,messages});}});
...
describe("durable command runtime",()=>{
  ...
  it("recovers persisted pending ownership after restart without duplicate reassignment",async()=>{
    ...
    const restarted=new CommandRuntime({store:api.store,clock,stage1Ms:10,stage2Ms:20,roster:()=>roster(),canLaunch:()=>true,executeTask:async(agent,_prompt,hooks)=>{await hooks.active("recovered");return{generationId:"recovered",visibleMessages:[agent]};},executePov:async()=>undefined,publishStatus:async(text)=>{api.statuses.push(text);},deliverTask:async(agent,messages)=>{api.deliveries.push({agent,messages});}});await restarted.initialize();await clock.tick(10);
