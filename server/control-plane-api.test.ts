...
describe("control-plane API authorization", () => {
  it("gates provider setup, enforces CSRF, delegates capabilities, and audits only redacted metadata", async () => {
    ...
    const ownerBody = await bootstrap.json() as { csrfToken: string; expiresAt: string };
    expect(bootstrap.headers.get("cache-control")).toBe("no-store");
    expect(Date.parse(ownerBody.expiresAt) - Date.now()).toBeGreaterThan(28_790_000);
    expect(Date.parse(ownerBody.expiresAt) - Date.now()).toBeLessThanOrEqual(28_800_000);
    const me = await call("/api/control/me", {}, ownerCookie);
    ...
    expect((await call("/api/control/logout", { method: "POST", body: "{}" }, ownerCookie)).status).toBe(403);
    expect((await call("/api/control/me", {}, ownerCookie)).status).toBe(200);
    const logout = await call("/api/control/logout", { method: "POST", body: "{}" }, ownerCookie, ownerBody.csrfToken);
    expect(logout.status).toBe(204);
    expect(logout.headers.get("set-cookie")).toContain("Max-Age=0");
    expect((await call("/api/control/me", {}, ownerCookie)).status).toBe(401);
    expect(await (await call("/api/control/status")).json()).toMatchObject({ claimed: true });
    const reauthenticated = await call("/api/control/login", { method: "POST", body: JSON.stringify({ username: "owner", password: "correct horse battery staple" }) });
    expect(reauthenticated.status).toBe(200);
    expect(await reauthenticated.json()).toMatchObject({ principal: { username: "owner" }, expiresAt: expect.any(String) });
  });
});
