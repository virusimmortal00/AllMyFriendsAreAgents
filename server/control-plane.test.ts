...
describe("durable control plane", () => {
  ...
  it("never writes bootstrap proofs, passwords, or arbitrary secret metadata to durable audit", async () => {
    ...
    await store.recordAudit(actor.id, "PROVIDER_SETUP_FAILED", "opencode", { harness: "opencode", status: "error", authorizationHeader: "Bearer audit-secret-sentinel", password });
    ...
    expect((await store.audit(actor)).at(-1)).toMatchObject({ action: "PROVIDER_SETUP_FAILED", metadata: { harness: "opencode", status: "error" } });
  });
});
