...
export function publicRoomState(state: RoomState, implementationCapabilities?: Partial<Record<AgentId, ImplementationCapability>>, viewerHumanId?: string): PublicRoomState {
  ...
  return { ...room, ...(roster ? { roster } : {}), messages, settings: publicSettings, ...(implementationCapabilities ? { implementationCapabilities } : {}) };
}
