  ...
  try {
    ...
    if (structuredOutput) {
      ...
      try {
        ...
        await journal?.append({ type: "generation.retry", generationId, agent, reason: "structured session was unavailable", staleSessionId: existing.id });
    ...
    if (error instanceof OpenCodeStructuredTurnCancelledError) {
      await journal?.append({ type: "generation.cancelled", generationId, agent, durationMs: Date.now() - startedAt, reason: error.message, transport: "sdk-server" });
