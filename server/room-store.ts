...
function validAttemptTransition(from: CommandAttempt["status"], to: CommandAttempt["status"]) {
  return (from === "queued" && ["active", "failed", "superseded"].includes(to))
    || (from === "active" && ["completed", "failed", "superseded"].includes(to));
}
...
export class RoomStore implements RoomRepository {
  ...
  async acceptCommand(acceptance: CommandAcceptance): Promise<AcceptCommandResult> {
    ...
    return this.mutateCommands<AcceptCommandResult>((state) => {
      ...
      return { next: { ...state, submissions: [...state.submissions, structuredClone(acceptance.submission)], audits: [...state.audits, structuredClone(acceptance.audit)], polls: acceptance.poll ? [...state.polls, structuredClone(acceptance.poll)] : state.polls, attempts: acceptance.attempt ? [...state.attempts, structuredClone(acceptance.attempt)] : state.attempts, povExecutions: acceptance.povExecution ? [...state.povExecutions, structuredClone(acceptance.povExecution)] : state.povExecutions, roundRobin: acceptance.roundRobin ? [...state.roundRobin.filter((item) => item.roomId !== acceptance.submission.roomId), structuredClone(acceptance.roundRobin.state)] : state.roundRobin }, result: { kind: "accepted" as const, acceptance: structuredClone(acceptance) } };
    });
  }
  ...
  async listPendingCommandAttempts(roomId: string) { await this.commandQueue; return structuredClone(this.commandState.attempts.filter((item) => item.roomId === roomId && (item.status === "queued" || item.status === "active")).sort((a, b) => a.createdAt.localeCompare(b.createdAt) || a.attemptId.localeCompare(b.attemptId))); }
  ...
  async listCommandPolls(roomId: string,query:{limit?:number;before?:string}={}) { await this.commandQueue;const limit=Math.max(1,Math.min(MAX_RECENT_POLLS,query.limit||50)); return structuredClone(this.commandState.polls.filter((poll) => poll.roomId === roomId&&(!query.before||poll.createdAt<query.before)).sort((a,b) => b.createdAt.localeCompare(a.createdAt) || b.pollId.localeCompare(a.pollId)).slice(0,limit)); }
  ...
  async createCommandVote(vote: CommandVote) { if (!validVote(vote)) throw new Error("Invalid command vote"); return this.mutateCommands<CreateCommandVoteResult>((state) => { const poll = state.polls.find((item) => item.roomId === vote.roomId && item.pollId === vote.pollId); if (!poll || vote.optionIndex >= poll.options.length) return { result: { kind: "rejected" as const, reason: "Poll or option does not exist in this room." } }; const duplicate = state.votes.find((item) => item.roomId === vote.roomId && item.pollId === vote.pollId && (item.voterId === vote.voterId || item.mutationId === vote.mutationId)); return duplicate ? { result: { kind: "duplicate" as const, vote: structuredClone(duplicate) } } : { next: { ...state, votes: [...state.votes, structuredClone(vote)] }, result: { kind: "created" as const, vote: structuredClone(vote) } }; }); }
  ...
  async compactCommandRecords(roomId:string,now:string){const cutoff=Date.parse(now)-COMMAND_RECORD_RETENTION_MS;await this.mutateCommands<void>((state)=>{const scoped=state.submissions.filter((item)=>item.roomId===roomId).sort((a,b)=>b.createdAt.localeCompare(a.createdAt));const pending=new Set([...state.attempts.filter((item)=>item.roomId===roomId&&(item.status==="queued"||item.status==="active")),...state.povExecutions.filter((item)=>item.roomId===roomId&&(item.status==="queued"||item.status==="active"))].map((item)=>item.submissionId));const keep=new Set(scoped.filter((item,index)=>pending.has(item.submissionId)||(index<MAX_COMMAND_SUBMISSIONS_PER_ROOM&&Date.parse(item.createdAt)>=cutoff)).map((item)=>item.submissionId));const removed=scoped.filter((item)=>!keep.has(item.submissionId));if(!removed.length)return{result:undefined};const removedIds=new Set(removed.map((item)=>item.submissionId));const pollIds=new Set(state.polls.filter((item)=>removedIds.has(item.submissionId)).map((item)=>item.pollId));const tombstones=[...state.tombstones,...removed.map((item)=>({roomId,submissionId:item.submissionId,clientSubmissionId:item.clientSubmissionId,command:item.command,compactedAt:now}))].filter((item)=>item.roomId!==roomId||!removed.some((candidate)=>candidate.submissionId===item.submissionId)||true).sort((a,b)=>b.compactedAt.localeCompare(a.compactedAt)).slice(0,MAX_COMMAND_TOMBSTONES_PER_ROOM);return{next:{...state,submissions:state.submissions.filter((item)=>!removedIds.has(item.submissionId)),attempts:state.attempts.filter((item)=>!removedIds.has(item.submissionId)),povExecutions:state.povExecutions.filter((item)=>!removedIds.has(item.submissionId)),polls:state.polls.filter((item)=>!removedIds.has(item.submissionId)),votes:state.votes.filter((item)=>!pollIds.has(item.pollId)),audits:state.audits.filter((item)=>!removedIds.has(item.submissionId)),tombstones},result:undefined};});}
