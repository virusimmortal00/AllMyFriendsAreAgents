...
export class RoomStore implements RoomRepository {
  ...
  async listPendingGhExecutions(roomId:string){await this.commandQueue;return structuredClone(this.commandState.ghExecutions.filter((item)=>item.roomId===roomId&&item.status==="queued").sort((a,b)=>a.createdAt.localeCompare(b.createdAt)||a.executionId.localeCompare(b.executionId)));}
