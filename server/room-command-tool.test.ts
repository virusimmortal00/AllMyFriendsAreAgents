...
async function fixture() {
  ...
  const broker=new RoomCommandToolBroker(runtime);const token=broker.issue({agentId:"codex-sol",displayName:"Sol",attempt:attempt(),allowedCommands:["help","poll"]});return{store,runtime,broker,token};
}
...
describe("server-owned room_command broker",()=>{
  it("projects safe command-family lease audit without duplicate or argument records",async()=>{
    ...
    const token=broker.issue({agentId:"codex-sol",displayName:"Sol",attempt:attempt(),allowedCommands:["help"]});
    expect(observed[0]).toMatchObject({outcome:"issued",reason:"lease-issued",command:null,selectorFamily:null});
    expect(broker.snapshot("codex-sol")).toMatchObject({present:true,status:"active",effectiveCommands:["help"],lastManifestIssuance:{revision:1}});
    ...
    const ghToken=broker.issue({agentId:"codex-sol",displayName:"Sol",attempt:attempt(),allowedCommands:["gh"]});
    ...
    const refreshed=broker.issue({agentId:"codex-sol",displayName:"Sol",attempt:attempt(()=>session==="session-a"),allowedCommands:["help"]});expect(observed.some(({outcome})=>outcome==="refreshed")).toBe(true);
    session="session-b";expect(broker.snapshot("codex-sol")).toMatchObject({present:false,status:"revoked",effectiveCommands:[]});expect(await broker.execute(refreshed,request)).toBeUndefined();
    expect(observed.filter(({outcome,reason})=>outcome==="revoked"&&reason==="generation-attempt-stale")).toHaveLength(1);
    broker.issue({agentId:"codex-sol",displayName:"Sol",attempt:attempt(),allowedCommands:["help"]});now+=10*60_000;expect(broker.snapshot("codex-sol")).toMatchObject({present:false,status:"expired",effectiveCommands:[]});
  ...
  it("binds identity and request IDs, supports only exact transport replay, and rechecks revocation",async()=>{const api=await fixture();const input={invocation:{command:"help" as const},clientSubmissionId:"agent-tool-help-01"};const first=await api.broker.execute(api.token,input);expect(first).toMatchObject({kind:"private-help",commands:["poll","help"],duplicate:false});expect(await api.broker.execute(api.token,input)).toEqual(first);expect(await api.broker.execute(api.token,{invocation:{command:"poll",question:"Substituted",options:["A","B"]},clientSubmissionId:"agent-tool-help-01"})).toEqual({kind:"private-error",message:"That request ID is already bound to a different room command."});expect(await api.broker.execute(api.token,{invocation:{command:"poll",question:"Different",options:["A","B"]},clientSubmissionId:"agent-tool-poll-01"})).toMatchObject({kind:"accepted",duplicate:false});expect(await api.broker.execute(api.token,{invocation:{command:"polls"},clientSubmissionId:"short"})).toEqual({kind:"private-error",message:"A valid command request ID is required."});
    const next=api.store.snapshot().roster!.entries.map((entry)=>entry.agentId==="codex-sol"?{...entry,commandPermissions:{allowAll:false,allowed:["help" as const]}}:entry);await api.store.updateRoster(2,next);const revokedToken=api.broker.issue({agentId:"codex-sol",displayName:"Sol",attempt:attempt(),allowedCommands:["help","poll"]});expect(await api.broker.execute(revokedToken,{invocation:{command:"poll",question:"No",options:["A","B"]},clientSubmissionId:"agent-tool-poll-02"})).toEqual({kind:"private-error",message:"That command is not available to this participant."});
  });

  it("routes agent commands by the server-issued room lease and ignores caller room fields",async()=>{const submit=vi.fn(async()=>({kind:"accepted" as const,submissionId:"bound",duplicate:false}));const runtime={submit} as unknown as CommandRuntime;const resolved:string[]=[];const broker=new RoomCommandToolBroker(async(roomId)=>{resolved.push(roomId);return runtime;});const token=broker.issue({agentId:"codex-sol",displayName:"Sol",attempt:attempt(),allowedCommands:["gh"],roomId:"server-room-a"});expect(await broker.execute(token,{invocation:{command:"gh",selector:{kind:"pr",number:98},roomId:"forged-room"} as never,clientSubmissionId:"bound-room-gh-01"})).toMatchObject({kind:"accepted"});expect(resolved).toEqual(["server-room-a"]);expect(submit).toHaveBeenCalledTimes(1);});
  ...
  it("revokes completed attempt leases before runtime or cached command data is reachable",async()=>{const api=await fixture();let session:string|null="session-a";const broker=new RoomCommandToolBroker(api.runtime,Date.now);const token=broker.issue({agentId:"codex-sol",displayName:"Sol",attempt:attempt(()=>session==="session-a"),allowedCommands:["help"]});session="session-b";expect(await broker.execute(token,{invocation:{command:"help"},clientSubmissionId:"stale-session-help-01"})).toBeUndefined();expect(await api.store.listCommandAuditIdentities("00000000-0000-4000-8000-000000000001")).toEqual([]);});
});
