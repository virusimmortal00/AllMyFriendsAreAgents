...
describe("SQLite migrations", () => {
  ...
  it("creates the portable storage schema and is idempotent", async () => {
    ...
    try {
      ...
      expect(database.prepare("SELECT COUNT(*) AS count FROM schema_migrations").get()).toEqual({ count: 29 });
      expect((database.prepare("PRAGMA table_info(room_settings)").all() as Array<{ name: string }>).map(({ name }) => name)).toContain("intent_classifier_enabled");
      expect((database.prepare("PRAGMA table_info(messages)").all() as Array<{ name: string }>).map(({ name }) => name)).toContain("recipient_human_id");
