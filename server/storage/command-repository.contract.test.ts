...
import { commandPollCursor, MAX_OPEN_POLLS_PER_ROOM, type CommandSubmission } from "../command-record.js";
...
describe.each(factories)("%s command repository", (_backend, makeFixture) => {
  ...
  it("atomically bounds retained open polls under concurrent-abuse pressure", async () => {
    ...
    try {
      for (let index = 0; index < MAX_OPEN_POLLS_PER_ROOM; index += 1) {
        const current = { ...submission(), submissionId: `cap-submission-${index}`, clientSubmissionId: `cap-client-${index}` };
        const poll = { pollId: `cap-poll-${index}`, roomId: DEFAULT_ROOM_ID, submissionId: current.submissionId, question: `Question ${index}`, options: ["A", "B"] as const, ...openPollLifecycle, createdAt: current.createdAt };
        const audit = { auditId: `cap-audit-${index}`, roomId: DEFAULT_ROOM_ID, submissionId: current.submissionId, command: "poll" as const, invokerKind: "human" as const, invokerId: "human-1", targetAgentIds: [] as const, createdAt: current.createdAt };
        expect(await fixture.repository.acceptCommand({ submission: current, poll, audit })).toMatchObject({ kind: "accepted" });
      }
      const overflow = { ...submission(), submissionId: "cap-submission-overflow", clientSubmissionId: "cap-client-overflow" };
      const overflowPoll = { pollId: "cap-poll-overflow", roomId: DEFAULT_ROOM_ID, submissionId: overflow.submissionId, question: "Overflow", options: ["A", "B"] as const, ...openPollLifecycle, createdAt: overflow.createdAt };
      const overflowAudit = { auditId: "cap-audit-overflow", roomId: DEFAULT_ROOM_ID, submissionId: overflow.submissionId, command: "poll" as const, invokerKind: "human" as const, invokerId: "human-1", targetAgentIds: [] as const, createdAt: overflow.createdAt };
      expect(await fixture.repository.acceptCommand({ submission: overflow, poll: overflowPoll, audit: overflowAudit })).toMatchObject({ kind: "rejected" });
      expect(await fixture.repository.getCommandSubmission(DEFAULT_ROOM_ID, overflow.submissionId)).toBeUndefined();
      expect(await fixture.repository.getCommandPoll(DEFAULT_ROOM_ID, overflowPoll.pollId)).toBeUndefined();
    } finally { fixture.close(); }
  });
  ...
  it("atomically closes polls, freezes one tally, and preserves idempotency across restart",async()=>{
    const root=await mkdtemp(path.join(os.tmpdir(),"amfaa-command-contract-"));temporaryDirectories.push(root);const fixture=await makeFixture(root);
    try{
      const value=submission();const poll={pollId:"poll-close",roomId:DEFAULT_ROOM_ID,submissionId:value.submissionId,question:"Choose",options:["A","B"] as const,...openPollLifecycle,createdAt:value.createdAt};
      const audit={auditId:"poll-close-audit",roomId:DEFAULT_ROOM_ID,submissionId:value.submissionId,command:"poll" as const,invokerKind:"human" as const,invokerId:"human-1",targetAgentIds:[] as const,createdAt:value.createdAt};
      await fixture.repository.acceptCommand({submission:value,poll,audit});
      const vote={roomId:DEFAULT_ROOM_ID,pollId:poll.pollId,voterId:"human:one",mutationId:"vote-before-close",optionIndex:1,createdAt:"2026-08-27T12:01:00.000Z"};
      expect(await fixture.repository.createCommandVote(vote)).toMatchObject({kind:"created"});
      const close={roomId:DEFAULT_ROOM_ID,pollId:poll.pollId,expectedRevision:1,mutationId:"close-mutation-one",closerKind:"human" as const,closerId:"human-1",closedAt:"2026-08-27T12:02:00.000Z"};
      expect(await fixture.repository.closeCommandPoll(close)).toMatchObject({kind:"closed",poll:{state:"CLOSED",revision:2,finalTallies:[0,1],finalTotalVotes:1}});
      expect(await fixture.repository.closeCommandPoll(close)).toMatchObject({kind:"duplicate",poll:{revision:2}});
      expect(await fixture.repository.createCommandVote(vote)).toMatchObject({kind:"duplicate"});
      expect(await fixture.repository.createCommandVote({...vote,voterId:"human:late",mutationId:"vote-after-close"})).toMatchObject({kind:"rejected"});
      const reopened=await fixture.reopen();expect(await reopened.getCommandPoll(DEFAULT_ROOM_ID,poll.pollId)).toMatchObject({state:"CLOSED",revision:2,closeMutationId:"close-mutation-one",finalTallies:[0,1]});
    }finally{fixture.close();}
  });

  it("atomically accepts a poll with its audit identity",async()=>{const root=await mkdtemp(path.join(os.tmpdir(),"amfaa-command-contract-"));temporaryDirectories.push(root);const fixture=await makeFixture(root);try{const value=submission();const poll={pollId:"poll-atomic",roomId:DEFAULT_ROOM_ID,submissionId:value.submissionId,question:"Choose",options:["A","B"] as const,...openPollLifecycle,createdAt:value.createdAt};const audit={auditId:"poll-audit",roomId:DEFAULT_ROOM_ID,submissionId:value.submissionId,command:"poll" as const,invokerKind:"human" as const,invokerId:"human-1",targetAgentIds:[] as const,createdAt:value.createdAt};expect(await fixture.repository.acceptCommand({submission:value,poll,audit})).toMatchObject({kind:"accepted"});const reopened=await fixture.reopen();expect(await reopened.getCommandPoll(DEFAULT_ROOM_ID,poll.pollId)).toEqual(poll);expect(await reopened.getCommandAuditIdentity(DEFAULT_ROOM_ID,value.submissionId)).toEqual(audit);}finally{fixture.close();}});

  it("persists per-target POV progress and keeps concurrent SQLite acceptance transactional",async()=>{const root=await mkdtemp(path.join(os.tmpdir(),"amfaa-command-contract-"));temporaryDirectories.push(root);const fixture=await makeFixture(root);try{const value={...submission(),command:"pov" as const,invocation:{command:"pov" as const,prompt:"Discuss",selection:{kind:"all-eligible" as const}}};const audit={auditId:"pov-audit",roomId:DEFAULT_ROOM_ID,submissionId:value.submissionId,command:"pov" as const,invokerKind:"human" as const,invokerId:"human-1",targetAgentIds:["codex-sol" as const,"claude-sonnet" as const],createdAt:value.createdAt};const pov={executionId:"pov-execution",roomId:DEFAULT_ROOM_ID,submissionId:value.submissionId,targetAgentIds:audit.targetAgentIds,processedTargetAgentIds:[] as ActiveAgentId[],status:"queued" as const,reason:null,createdAt:value.createdAt,updatedAt:value.createdAt};expect(await fixture.repository.acceptCommand({submission:value,audit,povExecution:pov})).toMatchObject({kind:"accepted"});const active={...pov,status:"active" as const,currentTargetAgentId:"codex-sol" as const,generationId:"pov-generation",deliveryMessages:["durable view"],deliveryResult:{sessionId:"pov-session",permission:"writable" as const},updatedAt:"2026-08-27T12:01:00.000Z"};expect(await fixture.repository.compareAndSetPovExecution(pov.updatedAt,active)).toMatchObject({kind:"accepted"});const reopened=await fixture.reopen();expect(await reopened.getPovExecution(DEFAULT_ROOM_ID,value.submissionId)).toEqual(active);
      if(_backend==="SQLite"){const make=(index:number)=>{const submissionValue={...submission(),submissionId:`concurrent-${index}`,clientSubmissionId:`concurrent-client-${index}`,command:"task" as const,invocation:{command:"task" as const,prompt:"work",selection:{kind:"round-robin" as const}}};return{submission:submissionValue,audit:{...audit,auditId:`concurrent-audit-${index}`,submissionId:submissionValue.submissionId,command:"task" as const,targetAgentIds:["codex-sol" as const]},attempt:{attemptId:`concurrent-attempt-${index}`,roomId:DEFAULT_ROOM_ID,submissionId:submissionValue.submissionId,attempt:1,agentId:"codex-sol" as const,generationId:null,status:"queued" as const,reason:null,createdAt:submissionValue.createdAt,updatedAt:submissionValue.createdAt},roundRobin:{expectedRevision:0,state:{roomId:DEFAULT_ROOM_ID,lastAssignedAgentId:"codex-sol" as const,revision:1,updatedAt:submissionValue.createdAt}}};};const settled=await Promise.allSettled([reopened.acceptCommand(make(1)),reopened.acceptCommand(make(2))]);expect(settled.every((result)=>result.status==="fulfilled")).toBe(true);}
    }finally{fixture.close();}});
  ...
  it("persists attempts, audit identities, and bounded private diagnostics idempotently", async () => {
    ...
    try {
      ...
      expect(await reopened.listDiagnostics(DEFAULT_ROOM_ID, { agentId: "codex-sol", search: "%" })).toEqual([]);
      expect(await reopened.listDiagnostics(DEFAULT_ROOM_ID, { agentId: "codex-sol", search: "_" })).toEqual([]);
      expect(await reopened.listDiagnostics("other-room", "codex-sol")).toEqual([]);
    } finally { fixture.close(); }
  });

  it("compacts expired command families into replay tombstones and paginates recent polls",async()=>{const root=await mkdtemp(path.join(os.tmpdir(),"amfaa-command-contract-"));temporaryDirectories.push(root);const fixture=await makeFixture(root);try{const old={...submission(),createdAt:"2020-01-01T00:00:00.000Z"};const oldPoll={pollId:"poll-old",roomId:DEFAULT_ROOM_ID,submissionId:old.submissionId,question:"Old",options:["A","B"] as const,...openPollLifecycle,createdAt:old.createdAt};const oldAudit={auditId:"audit-old",roomId:DEFAULT_ROOM_ID,submissionId:old.submissionId,command:"poll" as const,invokerKind:"human" as const,invokerId:"human-1",targetAgentIds:[] as const,createdAt:old.createdAt};expect(await fixture.repository.acceptCommand({submission:old,poll:oldPoll,audit:oldAudit})).toMatchObject({kind:"accepted"});expect(await fixture.repository.closeCommandPoll({roomId:DEFAULT_ROOM_ID,pollId:oldPoll.pollId,expectedRevision:1,mutationId:"old-poll-close",closerKind:"human",closerId:"human-1",closedAt:"2020-01-02T00:00:00.000Z"})).toMatchObject({kind:"closed"});await fixture.repository.compactCommandRecords(DEFAULT_ROOM_ID,"2026-08-27T12:00:00.000Z");expect(await fixture.repository.getCommandSubmission(DEFAULT_ROOM_ID,old.submissionId)).toBeUndefined();expect(await fixture.repository.getCommandPoll(DEFAULT_ROOM_ID,oldPoll.pollId)).toBeUndefined();expect(await fixture.repository.acceptCommand({submission:old,poll:oldPoll,audit:oldAudit})).toMatchObject({kind:"compacted-duplicate",tombstone:{clientSubmissionId:old.clientSubmissionId}});
      for(let index=0;index<3;index++){const createdAt="2026-08-27T12:02:00.000Z";const value={...submission(),submissionId:`recent-${index}`,clientSubmissionId:`recent-client-${index}`,createdAt};const poll={pollId:`recent-poll-${index}`,roomId:DEFAULT_ROOM_ID,submissionId:value.submissionId,question:`Recent ${index}`,options:["A","B"] as const,...openPollLifecycle,createdAt};const audit={...oldAudit,auditId:`recent-audit-${index}`,submissionId:value.submissionId,createdAt};expect(await fixture.repository.acceptCommand({submission:value,poll,audit})).toMatchObject({kind:"accepted"});}const first=await fixture.repository.listCommandPolls(DEFAULT_ROOM_ID,{limit:2});expect(first.map((item)=>item.pollId)).toEqual(["recent-poll-2","recent-poll-1"]);expect((await fixture.repository.listCommandPolls(DEFAULT_ROOM_ID,{limit:2,before:commandPollCursor(first[1]!)})).map((item)=>item.pollId)).toEqual(["recent-poll-0"]);expect(await fixture.repository.closeCommandPoll({roomId:DEFAULT_ROOM_ID,pollId:"recent-poll-2",expectedRevision:1,mutationId:"recent-poll-close-2",closerKind:"human",closerId:"human-1",closedAt:"2026-08-27T12:03:00.000Z"})).toMatchObject({kind:"closed"});expect((await fixture.repository.listCommandPolls(DEFAULT_ROOM_ID,{limit:2,state:"OPEN"})).map((item)=>item.pollId)).toEqual(["recent-poll-1","recent-poll-0"]);
    }finally{fixture.close();}});

  it("keeps another room's matching command identifiers during JSON compaction",async()=>{if(_backend!=="JSON")return;const root=await mkdtemp(path.join(os.tmpdir(),"amfaa-command-contract-"));temporaryDirectories.push(root);const fixture=await makeFixture(root);try{const old={...submission(),submissionId:"shared-submission",clientSubmissionId:"default-client",createdAt:"2020-01-01T00:00:00.000Z"};const other={...old,roomId:"other-room",clientSubmissionId:"other-client"};await fixture.repository.createCommandSubmission(old);await fixture.repository.createCommandPoll({pollId:"shared-poll",roomId:DEFAULT_ROOM_ID,submissionId:old.submissionId,question:"Default",options:["A","B"],...openPollLifecycle,createdAt:old.createdAt});await fixture.repository.createCommandSubmission(other);await fixture.repository.createCommandPoll({pollId:"shared-poll",roomId:"other-room",submissionId:other.submissionId,question:"Other",options:["A","B"],...openPollLifecycle,createdAt:other.createdAt});await fixture.repository.createCommandVote({roomId:"other-room",pollId:"shared-poll",voterId:"other-voter",mutationId:"other-vote",optionIndex:1,createdAt:other.createdAt});await fixture.repository.compactCommandRecords(DEFAULT_ROOM_ID,"2026-08-27T12:00:00.000Z");expect(await fixture.repository.getCommandSubmission("other-room","shared-submission")).toEqual(other);expect(await fixture.repository.getCommandPoll("other-room","shared-poll")).toMatchObject({question:"Other"});expect(await fixture.repository.listCommandVotes("other-room","shared-poll")).toHaveLength(1);}finally{fixture.close();}});
});
