...
export class SqliteRoomRepository implements RoomRepository {
  ...
  async compareAndSetContinuationInbox(expectedRevision: number, entry: ContinuationInboxEntry): Promise<CasResult<ContinuationInboxEntry>> {
    ...
    const before = await this.getContinuationInboxEntry(value.inboxEntryId); if (!before) return { kind: "not_found" }; if (before.inboxRevision !== expectedRevision || value.status === "ARCHIVED" || !canTransitionContinuationInbox(before.status, value.status)) return { kind: "conflict", actualRevision: before.inboxRevision }; if (value.roomId !== DEFAULT_ROOM_ID || !continuationInboxProvenanceMatches(before, value)) throw new Error("Continuation inbox provenance is immutable");
