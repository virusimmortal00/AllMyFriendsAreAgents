...
import { MAX_DIAGNOSTICS_PER_ROOM_AGENT, type CommandAttempt, type CommandAuditIdentity, type CommandPoll, type CommandSubmission, type CommandVote, type DiagnosticRecord, type RoundRobinState } from "../command-record.js";
...
export class SqliteRoomRepository implements RoomRepository {
  ...
  async listDiagnostics(roomId: string, agentId: ActiveAgentId, limit = 50) { return (this.database.prepare("SELECT * FROM command_diagnostics WHERE room_id=? AND agent_id=? ORDER BY created_at DESC,record_id DESC LIMIT ?").all(roomId,agentId,Math.max(1,Math.min(200,limit))) as Record<string, unknown>[]).map(diagnosticFromRow); }
