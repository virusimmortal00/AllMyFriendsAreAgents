...
export class SqliteRoomRepository implements RoomRepository {
  ...
  async createGhExecution(value:CommandGhExecution){if(!validGhExecution(value))throw new Error("Invalid GitHub execution");const row=this.database.prepare("SELECT * FROM command_gh_executions WHERE room_id=? AND (execution_id=? OR submission_id=?)").get(value.roomId,value.executionId,value.submissionId) as Record<string,unknown>|undefined;if(row)return{kind:"duplicate" as const,execution:ghExecutionFromRow(row)};this.database.prepare("INSERT INTO command_gh_executions(execution_id,room_id,submission_id,status,projection_json,rendered_text,failure_kind,diagnostics_json,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)").run(value.executionId,value.roomId,value.submissionId,value.status,value.projection?JSON.stringify(value.projection):null,value.renderedText,value.failureKind,JSON.stringify(value.diagnostics),value.createdAt,value.updatedAt);return{kind:"created" as const,execution:structuredClone(value)};}
  ...
  async listPendingGhExecutions(roomId:string){return(this.database.prepare("SELECT * FROM command_gh_executions WHERE room_id=? AND status='queued' ORDER BY created_at,execution_id").all(roomId) as Record<string,unknown>[]).map(ghExecutionFromRow);}
...
function ghExecutionFromRow(row:Record<string,unknown>):CommandGhExecution{return{executionId:String(row.execution_id),roomId:String(row.room_id),submissionId:String(row.submission_id),status:row.status as CommandGhExecution["status"],projection:row.projection_json==null?null:parseJson(String(row.projection_json),null),renderedText:row.rendered_text==null?null:String(row.rendered_text),failureKind:row.failure_kind==null?null:row.failure_kind as CommandGhExecution["failureKind"],diagnostics:parseJson(String(row.diagnostics_json),[]),createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
