...
import { COMMAND_RECORD_RETENTION_MS, DIAGNOSTIC_RETENTION_MS, MAX_COMMAND_SUBMISSIONS_PER_ROOM, MAX_COMMAND_TOMBSTONES_PER_ROOM, MAX_DIAGNOSTICS_PER_ROOM_AGENT, MAX_DIAGNOSTIC_QUERY_LIMIT, MAX_DIAGNOSTIC_SEARCH_LENGTH, MAX_RECENT_POLLS, parseCommandPollCursor, type AcceptCommandResult, type CommandAcceptance, type CommandAttempt, type CommandAuditIdentity, type CommandPoll, type CommandPovExecution, type CommandReassignment, type CommandSubmission, type CommandVote, type DiagnosticQuery, type DiagnosticRecord, type RoundRobinState } from "../command-record.js";
...
export class SqliteRoomRepository implements RoomRepository {
  ...
  async listCommandPolls(roomId: string,query:{limit?:number;before?:string}={}) { const limit=Math.max(1,Math.min(MAX_RECENT_POLLS,query.limit||50));const before=parseCommandPollCursor(query.before);return (this.database.prepare("SELECT * FROM command_polls WHERE room_id=? AND (? IS NULL OR created_at<? OR (created_at=? AND poll_id<?)) ORDER BY created_at DESC,poll_id DESC LIMIT ?").all(roomId,before?.createdAt||null,before?.createdAt||null,before?.createdAt||null,before?.pollId||null,limit) as Record<string,unknown>[]).map(pollFromRow); }
