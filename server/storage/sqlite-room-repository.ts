...
function povExecutionFromRow(row:Record<string,unknown>):CommandPovExecution{return{executionId:String(row.execution_id),roomId:String(row.room_id),submissionId:String(row.submission_id),targetAgentIds:parseJson(String(row.target_agent_ids_json),[]),processedTargetAgentIds:parseJson(String(row.processed_target_agent_ids_json),[]),status:row.status as CommandPovExecution["status"],reason:row.reason==null?null:String(row.reason),createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
