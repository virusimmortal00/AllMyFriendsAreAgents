...
import { DIAGNOSTIC_RETENTION_MS, MAX_DIAGNOSTICS_PER_ROOM_AGENT, MAX_DIAGNOSTIC_QUERY_LIMIT, MAX_DIAGNOSTIC_SEARCH_LENGTH, type AcceptCommandResult, type CommandAcceptance, type CommandAttempt, type CommandAuditIdentity, type CommandPoll, type CommandReassignment, type CommandSubmission, type CommandVote, type DiagnosticQuery, type DiagnosticRecord, type RoundRobinState } from "../command-record.js";
import { validAttempt, validAudit, validCommandAcceptance, validCommandReassignment, validDiagnostic, validPoll, validRoundRobin, validSubmission, validVote } from "./command-storage.js";
...
export class SqliteRoomRepository implements RoomRepository {
  ...
  async reassignCommandAttempt(value: CommandReassignment) {
    ...
    try {
      ...
      this.database.prepare("INSERT INTO command_attempts(attempt_id,room_id,submission_id,attempt,agent_id,generation_id,status,reason,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)").run(next.attemptId, next.roomId, next.submissionId, next.attempt, next.agentId, next.generationId, next.status, next.reason, next.createdAt, next.updatedAt);
  ...
  async createCommandAttempt(value: CommandAttempt) { if (!validAttempt(value)) throw new Error("Invalid command attempt"); const row = this.database.prepare("SELECT * FROM command_attempts WHERE room_id=? AND (attempt_id=? OR (submission_id=? AND attempt=?))").get(value.roomId,value.attemptId,value.submissionId,value.attempt) as Record<string, unknown> | undefined; if (row) return { kind: "duplicate" as const, attempt: attemptFromRow(row) }; this.database.prepare("INSERT INTO command_attempts(attempt_id,room_id,submission_id,attempt,agent_id,generation_id,status,reason,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)").run(value.attemptId,value.roomId,value.submissionId,value.attempt,value.agentId,value.generationId,value.status,value.reason,value.createdAt,value.updatedAt); return { kind: "created" as const, attempt: structuredClone(value) }; }
  ...
  async listCommandPolls(roomId: string) { return (this.database.prepare("SELECT * FROM command_polls WHERE room_id=? ORDER BY created_at,poll_id").all(roomId) as Record<string,unknown>[]).map(pollFromRow); }
  ...
  async createCommandVote(value: CommandVote) { if (!validVote(value)) throw new Error("Invalid command vote"); const poll = await this.getCommandPoll(value.roomId,value.pollId); if (!poll || value.optionIndex >= poll.options.length) return { kind: "rejected" as const, reason: "Poll or option does not exist in this room." }; const row = this.database.prepare("SELECT * FROM command_poll_votes WHERE room_id=? AND poll_id=? AND voter_id=?").get(value.roomId,value.pollId,value.voterId) as Record<string, unknown> | undefined; if (row) return { kind: "duplicate" as const, vote: voteFromRow(row) }; this.database.prepare("INSERT INTO command_poll_votes(room_id,poll_id,voter_id,option_index,created_at) VALUES (?,?,?,?,?)").run(value.roomId,value.pollId,value.voterId,value.optionIndex,value.createdAt); return { kind: "created" as const, vote: structuredClone(value) }; }
...
function voteFromRow(row: Record<string, unknown>): CommandVote { return { roomId: String(row.room_id), pollId: String(row.poll_id), voterId: String(row.voter_id), optionIndex: Number(row.option_index), createdAt: String(row.created_at) }; }
