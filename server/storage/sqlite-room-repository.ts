...
import { COMMAND_RECORD_RETENTION_MS, DIAGNOSTIC_RETENTION_MS, MAX_COMMAND_SUBMISSIONS_PER_ROOM, MAX_COMMAND_TOMBSTONES_PER_ROOM, MAX_DIAGNOSTICS_PER_ROOM_AGENT, MAX_DIAGNOSTIC_QUERY_LIMIT, MAX_DIAGNOSTIC_SEARCH_LENGTH, MAX_RECENT_POLLS, type AcceptCommandResult, type CommandAcceptance, type CommandAttempt, type CommandAuditIdentity, type CommandPoll, type CommandPovExecution, type CommandReassignment, type CommandSubmission, type CommandVote, type DiagnosticQuery, type DiagnosticRecord, type RoundRobinState } from "../command-record.js";
import { validAttempt, validAudit, validCommandAcceptance, validCommandReassignment, validDiagnostic, validPoll, validPovExecution, validRoundRobin, validSubmission, validVote } from "./command-storage.js";
...
export class SqliteRoomRepository implements RoomRepository {
  ...
  async reassignCommandAttempt(value: CommandReassignment) {
    ...
    try {
      ...
      const pointer = await this.getRoundRobinState(value.current.roomId);
  ...
  async listPendingCommandAttempts(roomId: string) { return (this.database.prepare("SELECT * FROM command_attempts WHERE room_id=? AND status IN ('queued','active') ORDER BY created_at,attempt_id").all(roomId) as Record<string, unknown>[]).map(attemptFromRow); }
  async compareAndSetCommandAttempt(expectedUpdatedAt: string, value: CommandAttempt) { if (!validAttempt(value) || value.updatedAt === expectedUpdatedAt) throw new Error("Invalid command attempt transition"); const current = this.database.prepare("SELECT * FROM command_attempts WHERE room_id=? AND attempt_id=?").get(value.roomId,value.attemptId) as Record<string, unknown> | undefined; if (!current) return { kind: "not-found" as const }; const attempt = attemptFromRow(current); if (attempt.updatedAt !== expectedUpdatedAt || attempt.submissionId !== value.submissionId || attempt.attempt !== value.attempt || attempt.agentId !== value.agentId || attempt.createdAt !== value.createdAt || !validAttemptTransition(attempt.status,value.status)) return { kind: "conflict" as const }; const result = this.database.prepare("UPDATE command_attempts SET generation_id=?,status=?,reason=?,updated_at=? WHERE room_id=? AND attempt_id=? AND updated_at=?").run(value.generationId,value.status,value.reason,value.updatedAt,value.roomId,value.attemptId,expectedUpdatedAt); return result.changes === 1 ? { kind: "accepted" as const, attempt: structuredClone(value) } : { kind: "conflict" as const }; }
  async createCommandPoll(value: CommandPoll) { if (!validPoll(value)) throw new Error("Invalid command poll"); const row = this.database.prepare("SELECT * FROM command_polls WHERE room_id=? AND (poll_id=? OR submission_id=?)").get(value.roomId,value.pollId,value.submissionId) as Record<string, unknown> | undefined; if (row) return { kind: "duplicate" as const, poll: pollFromRow(row) }; this.database.prepare("INSERT INTO command_polls(poll_id,room_id,submission_id,question,options_json,created_at) VALUES (?,?,?,?,?,?)").run(value.pollId,value.roomId,value.submissionId,value.question,JSON.stringify(value.options),value.createdAt); return { kind: "created" as const, poll: structuredClone(value) }; }
  async listCommandPolls(roomId: string,query:{limit?:number;before?:string}={}) { const limit=Math.max(1,Math.min(MAX_RECENT_POLLS,query.limit||50));return (this.database.prepare("SELECT * FROM command_polls WHERE room_id=? AND (? IS NULL OR created_at<?) ORDER BY created_at DESC,poll_id DESC LIMIT ?").all(roomId,query.before||null,query.before||null,limit) as Record<string,unknown>[]).map(pollFromRow); }
  ...
  async createCommandVote(value: CommandVote) { if (!validVote(value)) throw new Error("Invalid command vote"); const poll = await this.getCommandPoll(value.roomId,value.pollId); if (!poll || value.optionIndex >= poll.options.length) return { kind: "rejected" as const, reason: "Poll or option does not exist in this room." }; const row = this.database.prepare("SELECT * FROM command_poll_votes WHERE room_id=? AND poll_id=? AND (voter_id=? OR mutation_id=?)").get(value.roomId,value.pollId,value.voterId,value.mutationId) as Record<string, unknown> | undefined; if (row) return { kind: "duplicate" as const, vote: voteFromRow(row) }; this.database.prepare("INSERT INTO command_poll_votes(room_id,poll_id,voter_id,mutation_id,option_index,created_at) VALUES (?,?,?,?,?,?)").run(value.roomId,value.pollId,value.voterId,value.mutationId,value.optionIndex,value.createdAt); return { kind: "created" as const, vote: structuredClone(value) }; }
...
function attemptFromRow(row: Record<string, unknown>): CommandAttempt { return { attemptId: String(row.attempt_id), roomId: String(row.room_id), submissionId: String(row.submission_id), attempt: Number(row.attempt), agentId: row.agent_id as ActiveAgentId, generationId: row.generation_id === null ? null : String(row.generation_id), status: row.status as CommandAttempt["status"], reason: row.reason === null ? null : String(row.reason), ...(row.room_epoch==null?{}:{roomEpoch:String(row.room_epoch)}), ...(row.roster_revision==null?{}:{rosterRevision:Number(row.roster_revision)}), ...(row.agent_configuration_revision==null?{}:{agentConfigurationRevision:Number(row.agent_configuration_revision)}), createdAt: String(row.created_at), updatedAt: String(row.updated_at) }; }
function validAttemptTransition(from: CommandAttempt["status"], to: CommandAttempt["status"]) { return from === "queued" && ["active","failed","superseded"].includes(to) || from === "active" && ["completed","failed","superseded"].includes(to); }
function pollFromRow(row: Record<string, unknown>): CommandPoll { return { pollId: String(row.poll_id), roomId: String(row.room_id), submissionId: String(row.submission_id), question: String(row.question), options: parseJson(String(row.options_json), []) as unknown as CommandPoll["options"], createdAt: String(row.created_at) }; }
...
function povExecutionFromRow(row:Record<string,unknown>):CommandPovExecution{return{executionId:String(row.execution_id),roomId:String(row.room_id),submissionId:String(row.submission_id),targetAgentIds:parseJson(String(row.target_agent_ids_json),[]),status:row.status as CommandPovExecution["status"],reason:row.reason==null?null:String(row.reason),createdAt:String(row.created_at),updatedAt:String(row.updated_at)};}
