...
export class SqliteRoomRepository implements RoomRepository {
  ...
  async compareAndSetContinuation(expectedRevision: number, record: ContinuationRecord, event: ContinuationAuditEvent): Promise<CasResult<ContinuationRecord>> {
    ...
    if (!auditMatches(audit, value, before.status)) throw new Error("Invalid continuation audit event");
  ...
  async completeContinuation(expectedRevision: number, record: ContinuationRecord, entry: ContinuationInboxEntry, maxEntries: number, event: ContinuationAuditEvent): Promise<CasResult<ContinuationRecord>> {
    const job = normalizeContinuationRecord(record); const inbox = normalizeContinuationInboxEntry(entry); const audit = normalizeContinuationAuditEvent(event); if (!job || !inbox || job.status !== "COMPLETED" || job.jobId !== inbox.jobId || job.jobRevision !== expectedRevision + 1) throw new Error("Invalid atomic completion");
    this.database.exec("BEGIN IMMEDIATE");
    try {
      ...
      if (!auditMatches(audit, job, before.status)) throw new Error("Invalid completion audit event");
      ...
      for (const stale of rows.map((row) => normalizeContinuationInboxEntry(parseJson(row.projection_json, undefined))).filter((v): v is ContinuationInboxEntry => Boolean(v)).slice(0, Math.max(0, rows.length - Math.max(1, maxEntries)))) { const archived = { ...stale, inboxRevision: stale.inboxRevision + 1, status: "ARCHIVED" as const, updatedAt: inbox.createdAt, closedAt: inbox.createdAt }; this.database.prepare("UPDATE continuation_inbox SET inbox_revision = ?, status = ?, projection_json = ?, updated_at = ? WHERE room_id = ? AND inbox_entry_id = ? AND inbox_revision = ?").run(archived.inboxRevision, archived.status, JSON.stringify(archived), archived.updatedAt, DEFAULT_ROOM_ID, archived.inboxEntryId, stale.inboxRevision); const staleJob = await this.getContinuation(stale.jobId); if (staleJob?.resultDisposition === "INBOX") { const archivedJob = { ...staleJob, jobRevision: staleJob.jobRevision + 1, resultDisposition: "ARCHIVED" as const, updatedAt: inbox.createdAt }; this.database.prepare("UPDATE continuation_jobs SET job_revision = ?, projection_json = ?, updated_at = ? WHERE room_id = ? AND job_id = ? AND job_revision = ?").run(archivedJob.jobRevision, JSON.stringify(archivedJob), archivedJob.updatedAt, DEFAULT_ROOM_ID, archivedJob.jobId, staleJob.jobRevision); this.insertContinuationAudit(capacityArchiveAudit(archivedJob, staleJob.status, inbox.createdAt)); } }
  ...
  async compareAndSetContinuationInbox(expectedRevision: number, entry: ContinuationInboxEntry): Promise<CasResult<ContinuationInboxEntry>> {
    ...
    const before = await this.getContinuationInboxEntry(value.inboxEntryId); if (!before) return { kind: "not_found" }; if (before.inboxRevision !== expectedRevision || value.status === "ARCHIVED" || !canTransitionContinuationInbox(before.status, value.status)) return { kind: "conflict", actualRevision: before.inboxRevision };
  ...
  async archiveContinuationInbox(expectedRevision: number, entry: ContinuationInboxEntry): Promise<CasResult<ContinuationInboxEntry>> {
    ...
    this.database.exec("BEGIN IMMEDIATE"); try { const before = await this.getContinuationInboxEntry(value.inboxEntryId); if (!before) { this.database.exec("ROLLBACK"); return { kind: "not_found" }; } if (before.inboxRevision !== expectedRevision || !canTransitionContinuationInbox(before.status, "ARCHIVED")) { this.database.exec("ROLLBACK"); return { kind: "conflict", actualRevision: before.inboxRevision }; } const job = await this.getContinuation(before.jobId); if (!job) { this.database.exec("ROLLBACK"); return { kind: "not_found" }; } const archivedJob = { ...job, jobRevision: job.jobRevision + 1, resultDisposition: "ARCHIVED" as const, updatedAt: value.updatedAt }; this.database.prepare("UPDATE continuation_inbox SET inbox_revision = ?, status = ?, projection_json = ?, updated_at = ? WHERE room_id = ? AND inbox_entry_id = ? AND inbox_revision = ?").run(value.inboxRevision, value.status, JSON.stringify(value), value.updatedAt, DEFAULT_ROOM_ID, value.inboxEntryId, expectedRevision); this.database.prepare("UPDATE continuation_jobs SET job_revision = ?, projection_json = ?, updated_at = ? WHERE room_id = ? AND job_id = ? AND job_revision = ?").run(archivedJob.jobRevision, JSON.stringify(archivedJob), archivedJob.updatedAt, DEFAULT_ROOM_ID, archivedJob.jobId, job.jobRevision); this.insertContinuationAudit(capacityArchiveAudit(archivedJob, job.status, value.updatedAt)); this.database.exec("COMMIT"); return { kind: "accepted", value: structuredClone(value) }; } catch (error) { this.database.exec("ROLLBACK"); throw error; }
  }
  importContinuations(policy: ContinuationPolicy | undefined, jobs: readonly ContinuationRecord[], inbox: readonly ContinuationInboxEntry[], events: readonly ContinuationAuditEvent[] = []) {
    ...
    try {
      ...
      for (const raw of events) { const event = normalizeContinuationAuditEvent(raw); if (!event) throw new Error("Invalid imported continuation audit event"); const existing = this.database.prepare("SELECT projection_json FROM continuation_job_events WHERE room_id = ? AND job_id = ? AND job_revision = ?").get(DEFAULT_ROOM_ID, event.jobId, event.jobRevision) as { projection_json: string } | undefined; if (existing && existing.projection_json !== JSON.stringify(event)) throw new Error(`Continuation audit ${event.eventId} diverges from SQLite`); if (!existing) this.insertContinuationAudit(event); }
      this.database.exec("COMMIT");
