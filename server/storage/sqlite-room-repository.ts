...
export class SqliteRoomRepository implements RoomRepository {
  ...
  async getStorageScope(roomId: string): Promise<StorageScope | undefined> {
    ...
    const attachment = this.database.prepare("SELECT attachment_revision FROM rooms WHERE id=?").get(roomId) as { attachment_revision?: number } | undefined;
    return { schemaVersion: 1, serverId: room.serverId, roomId: room.roomId, roomAttachmentRevision: Number(attachment?.attachment_revision || 0), projectId: room.projectId,
  ...
  async createGhExecution(value:CommandGhExecution){this.assertOwnedRoom(value.roomId);if(!validGhExecution(value))throw new Error("Invalid GitHub execution");let row=this.database.prepare("SELECT * FROM command_gh_executions WHERE room_id=? AND (execution_id=? OR submission_id=?)").get(value.roomId,value.executionId,value.submissionId) as Record<string,unknown>|undefined;if(row){if(row.status==="queued"&&row.authorization_lease==null&&value.authorizationLease)this.database.prepare("UPDATE command_gh_executions SET authorization_lease=? WHERE room_id=? AND submission_id=? AND status='queued' AND authorization_lease IS NULL").run(value.authorizationLease,value.roomId,value.submissionId);row=this.database.prepare("SELECT * FROM command_gh_executions WHERE room_id=? AND submission_id=?").get(value.roomId,value.submissionId) as Record<string,unknown>;return{kind:"duplicate" as const,execution:ghExecutionFromRow(row)}}this.database.prepare("INSERT INTO command_gh_executions(execution_id,room_id,submission_id,status,delivery_status,authorization_lease,projection_json,rendered_text,failure_kind,diagnostics_json,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)").run(value.executionId,value.roomId,value.submissionId,value.status,value.deliveryStatus,value.authorizationLease,value.projection?JSON.stringify(value.projection):null,value.renderedText,value.failureKind,JSON.stringify(value.diagnostics),value.createdAt,value.updatedAt);return{kind:"created" as const,execution:structuredClone(value)};}
