...
import { CONTINUATION_POLICY_VERSION, continuationProvenanceHash, finalizeContinuationAudit, projectPathHash, type ContinuationAuditAction, type ContinuationAuditEvent, type ContinuationInboxEntry, type ContinuationPolicy, type ContinuationRecord, type ContinuationRecordStore } from "../continuation-record.js";
...
async function fixtures(): Promise<Array<[string, ContinuationRecordStore, () => Promise<ContinuationRecordStore>]>> { const root = await mkdtemp(path.join(os.tmpdir(), "amfaa-continuations-")); roots.push(root); const jsonPath = path.join(root, "json"); const sqlitePath = path.join(root, "room.sqlite"); return [["json", await RoomStore.open(root, jsonPath), () => RoomStore.open(root, jsonPath)], ["sqlite", await SqliteRoomRepository.open(root, sqlitePath, { seedImprovements: false }), () => SqliteRoomRepository.open(root, sqlitePath, { seedImprovements: false })]]; }
...
describe("continuation repository contract", () => {
  it("has JSON/SQLite parity for policy CAS, one-slot races, job CAS, and atomic inbox completion", async () => {
    for (const [backend, store] of await fixtures()) {
      ...
      const dispatched = event(running, "QUEUED", "DISPATCHED");
      expect(await store.compareAndSetContinuation(1, running, dispatched), backend).toMatchObject({ kind: "accepted" });
      expect(await store.compareAndSetContinuation(1, running, dispatched), backend).toMatchObject({ kind: "conflict", actualRevision: 2 });
      const done = completed(running, 3); const inbox = entry("inbox-1", done);
      const completedEvent = event(done, "RUNNING", "COMPLETED");
      expect(await store.completeContinuation(2, done, inbox, 1, completedEvent), backend).toMatchObject({ kind: "accepted" });
      expect(await store.getContinuationInboxEntry("inbox-1"), backend).toMatchObject({ status: "UNREAD", jobId: winner.jobId });
      expect(await store.completeContinuation(2, done, inbox, 1, completedEvent), backend).toMatchObject({ kind: "conflict" });
  ...
  it("atomically archives capacity-evicted inbox and job projections while excluding CLOSED entries", async () => {
    for (const [backend, store, reopen] of await fixtures()) {
      ...
      if (store instanceof SqliteRoomRepository) store.close(); const reloaded = await reopen(); expect(await reloaded.getContinuationInboxEntry("capacity-inbox-1"), backend).toMatchObject({ status: "ARCHIVED", closedAt: now }); expect(await reloaded.getContinuation(first.jobId), backend).toMatchObject({ resultDisposition: "ARCHIVED", auditEventCount: 4 }); if (reloaded instanceof SqliteRoomRepository) reloaded.close();
    }
  });
  it("rejects cross-room, forged-audit, and immutable-provenance mutations with JSON/SQLite parity", async () => {
    for (const [backend, store] of await fixtures()) {
      ...
      for (const invalid of [{ ...running, startedAt: null }, { ...running, cancellationRequested: true }, { ...running, resultDisposition: "ARCHIVED" as const }, { ...running, resultSummary: "forged" }, { ...running, completedAt: "2026-08-25T00:00:00.000Z" }]) await expect(store.compareAndSetContinuation(1, invalid, event(invalid, "QUEUED", "DISPATCHED")), backend).rejects.toThrow(/audit event/);
      for (const forged of [{ ...dispatched, action: "FAILED" as const }, { ...dispatched, attempt: 9 }, { ...dispatched, trigger: "forged trigger" }, { ...dispatched, policyRevision: 9 }, { ...dispatched, provenanceHash: "b".repeat(64) }, { ...dispatched, usage: { ...dispatched.usage, tokens: 1 } }, { ...dispatched, attemptUsage: { ...dispatched.attemptUsage, toolCalls: 1 } }, { ...dispatched, result: "forged" }, { ...dispatched, at: "2026-08-24T12:01:00.000Z" }, { ...dispatched, nextEligibilityAt: now }]) await expect(store.compareAndSetContinuation(1, running, forged), backend).rejects.toThrow(/audit event/);
      expect(await store.compareAndSetContinuation(1, running, dispatched), backend).toMatchObject({ kind: "accepted" });
      ...
      for (const invalid of [{ ...done, cancellationRequested: true }, { ...done, blocker: "forged" }, { ...done, nextEligibilityAt: "2026-08-24T12:02:00.000Z" }, { ...done, resultDisposition: "CLOSED" as const }, { ...done, completedAt: "2026-08-25T00:00:00.000Z" }]) await expect(store.completeContinuation(5, invalid, { ...validInbox, summary: invalid.resultSummary! }, 2, event(invalid, "RUNNING", "COMPLETED")), backend).rejects.toThrow(/completion|audit/);
      const mismatchedResult = { ...done, resultSummary: "Other result" }; await expect(store.completeContinuation(5, mismatchedResult, validInbox, 2, event(mismatchedResult, "RUNNING", "COMPLETED")), backend).rejects.toThrow(/completion/);
      const completedEvent = event(done, "RUNNING", "COMPLETED");
      for (const mismatch of [{ ...validInbox, owner: "claude-sonnet" as AgentId }, { ...validInbox, task: { ...validInbox.task, taskId: "other-task" } }, { ...validInbox, taskRevision: validInbox.taskRevision + 1 }, { ...validInbox, assignmentId: "other-assignment" }, { ...validInbox, roomId: "foreign-room", task: { ...validInbox.task, roomId: "foreign-room" } }]) await expect(store.completeContinuation(5, done, mismatch, 2, completedEvent), backend).rejects.toThrow(/completion/);
      await store.completeContinuation(5, done, validInbox, 2, completedEvent);
      const acknowledged = { ...validInbox, inboxRevision: 2, status: "ACKNOWLEDGED" as const, acknowledgedAt: done.updatedAt };
      await expect(store.compareAndSetContinuationInbox(1, { ...acknowledged, acknowledgedAt: null }), backend).rejects.toThrow(/inbox/);
      await expect(store.compareAndSetContinuationInbox(1, { ...acknowledged, status: "CLOSED", closedAt: null }), backend).rejects.toThrow(/inbox/);
      ...
      await expect(store.archiveContinuationInbox(2, { ...archived, closedAt: null }), backend).rejects.toThrow(/archive/);
