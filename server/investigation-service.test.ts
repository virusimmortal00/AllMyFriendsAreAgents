...
async function fixture(executor: InvestigationExecutor = new DeferredExecutor(), options: { stopped?: () => boolean; maxConcurrentGlobal?: number; defaultTokenLimit?: number; now?: () => Date } = {}) {
  const root = await mkdtemp(path.join(os.tmpdir(), "amfaa-investigation-")); roots.push(root); const stateDirectory = path.join(root, "state"); const rooms = await RoomStore.open(root, path.join(root, "room")); const store = await InvestigationStore.open(stateDirectory); const service = new InvestigationService(store, rooms, executor, { configuredEnabled: false, ...(options.stopped?{emergencyStopped:options.stopped}:{}), ...(options.maxConcurrentGlobal===undefined?{}:{maxConcurrentGlobal:options.maxConcurrentGlobal}), ...(options.defaultTokenLimit===undefined?{}:{defaultTokenLimit:options.defaultTokenLimit}), ...(options.now?{now:options.now}:{}) }); services.push(service); await service.initialize(); const policy = (await service.policy())!; await service.updatePolicy(policy.revision, true, "test-human"); const message = requiredValue(rooms.snapshot().messages.at(-1), "investigation evidence message"); const evidence = [{ kind: "room_message" as const, ref: message.id }]; return { root, stateDirectory, rooms, store, service, executor, evidence };
}
...
describe("independent investigation lane", () => {
  it("runs beside foreground activity with a distinct session and lands only in the inbox", async () => {
    ...
    await eventually(() => executor.inputs.length === 1); const input = requiredAt(executor.inputs, 0, "foreground-isolated investigation dispatch"); expect(input.forbiddenProviderSessionIds).toContain("foreground-session"); expect(input.capabilities).toEqual(["READ_PROJECT", "READ_OBSERVABILITY", "RUN_READ_ONLY_TESTS"]); expect(input.excludedCapabilities).toContain("EDIT");
    await value.rooms.addMessage("you", "The room moved on while this ran."); expect(input.signal.aborted).toBe(false); executor.resolve({ providerSessionId: "background-session" }); await eventually(async () => (await value.service.list())[0]?.status === "COMPLETED");
  ...
  it("bounds global provider concurrency while allowing one independent lane for another agent", async () => {
    const value = await fixture(new DeferredExecutor(), { maxConcurrentGlobal: 1 }); const executor = value.executor as DeferredExecutor; await value.service.request({ owner: "codex-sol", objective: "First", trigger: "signal one", signal: "AGENT_DECISION", evidenceRefs: value.evidence }); await value.service.request({ owner: "claude-sonnet", objective: "Second", trigger: "signal two", signal: "AGENT_DECISION", evidenceRefs: value.evidence }); await eventually(() => executor.inputs.length === 1); expect((await value.service.list()).filter((job) => job.status === "RUNNING")).toHaveLength(1); expect((await value.service.list()).filter((job) => job.status === "QUEUED")).toHaveLength(1); executor.resolve({ providerSessionId: "first-background" }); await eventually(() => executor.inputs.length === 2); expect(requiredAt(executor.inputs, 1, "queued investigation dispatch").owner).toBe("claude-sonnet");
  });
  it("reserves global concurrency before concurrent queued jobs reach asynchronous dispatch", async () => {
    const value = await fixture(new DeferredExecutor(), { maxConcurrentGlobal: 1 }); const executor = value.executor as DeferredExecutor;
    await Promise.all([
      value.service.request({ owner: "codex-sol", objective: "Concurrent first", trigger: "signal one", signal: "AGENT_DECISION", evidenceRefs: value.evidence }),
      value.service.request({ owner: "claude-sonnet", objective: "Concurrent second", trigger: "signal two", signal: "AGENT_DECISION", evidenceRefs: value.evidence }),
    ]);
    await eventually(() => executor.inputs.length === 1); await new Promise((resolve) => setTimeout(resolve, 25)); expect(executor.inputs).toHaveLength(1); expect(value.service.activeCount()).toBe(1);
    executor.resolve({ providerSessionId: "reserved-first" }); await eventually(() => executor.inputs.length === 2); expect(value.service.activeCount()).toBe(1);
  });
  it("fails one of two concurrent investigations when the executor reuses a provider session", async () => {
    const value = await fixture(new DeferredExecutor(), { maxConcurrentGlobal: 2 }); const executor = value.executor as DeferredExecutor;
    await Promise.all([
      value.service.request({ owner: "codex-sol", objective: "First isolation check", trigger: "signal one", signal: "AGENT_DECISION", evidenceRefs: value.evidence }),
      value.service.request({ owner: "claude-sonnet", objective: "Second isolation check", trigger: "signal two", signal: "AGENT_DECISION", evidenceRefs: value.evidence }),
    ]);
    await eventually(() => executor.inputs.length === 2); executor.resolve({ providerSessionId: "reused-background-session" }); executor.resolve({ providerSessionId: "reused-background-session" });
    await eventually(async () => (await value.service.list()).every((job) => job.status === "COMPLETED" || job.status === "FAILED")); const jobs = await value.service.list(); expect(jobs.filter((job) => job.status === "COMPLETED")).toHaveLength(1); expect(jobs.filter((job) => job.status === "FAILED")).toHaveLength(1); expect(jobs.find((job) => job.status === "FAILED")?.blocker).toMatch(/provider session/i); expect((await value.service.inbox("codex-sol")).length + (await value.service.inbox("claude-sonnet")).length).toBe(1);
  });
  it("returns a waiting tool job to running before accepting its final result", async () => {
    const value = await fixture(); const executor = value.executor as DeferredExecutor; const requested = await value.service.request({ owner: "codex-sol", objective: "Finish after tool", trigger: "tool signal", signal: "AGENT_DECISION", evidenceRefs: value.evidence }); expect(requested.kind).toBe("ok"); await eventually(() => executor.inputs.length === 1);
    await requiredAt(executor.inputs, 0, "tool-bound investigation dispatch").progress("WAITING_TOOL", "Reading bounded evidence", { summary: "Evidence opened", opaqueState: "tool:complete" }); executor.resolve({ providerSessionId: "waiting-tool-session" }); await eventually(async () => (await value.service.list())[0]?.status === "COMPLETED");
    const audit = await value.service.audit(requested.kind === "ok" ? requested.value.investigationId : ""); expect(audit.slice(-2).map((event) => event.action)).toEqual(["TOOL_RESUMED", "COMPLETED"]); expect(await value.service.inbox("codex-sol")).toHaveLength(1);
  });
  ...
  it("persists tool-boundary checkpoints and reconciles restart without claiming an orphan is running", async () => {
    const value = await fixture(); const executor = value.executor as DeferredExecutor; await value.service.request({ owner: "codex-sol", objective: "Check", trigger: "signal", signal: "AGENT_DECISION", evidenceRefs: value.evidence }); await eventually(() => executor.inputs.length === 1); await requiredAt(executor.inputs, 0, "checkpointing investigation dispatch").progress("WAITING_TOOL", "Reading local tests", { summary: "Inspected parser", opaqueState: "next:test-file" }); expect((await value.service.list())[0]).toMatchObject({ status: "WAITING_TOOL", checkpoint: { summary: "Inspected parser" } });
    ...
    const reopenedStore = await InvestigationStore.open(value.stateDirectory); const restarted = new InvestigationService(reopenedStore, reopenedRooms, new DeferredExecutor()); services.push(restarted); await restarted.initialize(); const checkpoint = requiredAt(await restarted.list(), 0, "restarted investigation checkpoint"); expect(checkpoint).toMatchObject({ status: "CHECKPOINTED", blocker: expect.stringContaining("Ready to resume") }); await restarted.resume(checkpoint.investigationId); await eventually(() => restarted.activeCount() === 1);
  });
  ...
  it("rejects forged durable checkpoints and audit chains on reopen", async () => {
    const value = await fixture(); const executor = value.executor as DeferredExecutor; await value.service.request({ owner: "codex-sol", objective: "Check", trigger: "signal", signal: "AGENT_DECISION", evidenceRefs: value.evidence }); await eventually(() => executor.inputs.length === 1); await requiredAt(executor.inputs, 0, "forged-checkpoint investigation dispatch").progress("WAITING_TOOL", "tool", { summary: "checkpoint", opaqueState: "safe" }); const file = path.join(value.stateDirectory, "investigations.json"); const state = JSON.parse(await readFile(file, "utf8")); const jobId = requiredAt(Object.keys(state.jobs), 0, "persisted investigation job key"); state.jobs[jobId].checkpoint.opaqueState = "forged"; await writeFile(file, JSON.stringify(state)); await expect(InvestigationStore.open(value.stateDirectory)).rejects.toThrow(/checkpoint|investigation/i);
  });
});
