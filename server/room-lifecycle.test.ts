...
describe("durable room lifecycle",()=>{
  it("resolves membership server-side and isolates transcripts, rosters, tasks, sessions, and identities",async()=>{const value=await fixture();const a=value.lifecycle.create("human-a",{name:"Alpha"});const b=value.lifecycle.create("human-a",{name:"Beta"});expect(value.lifecycle.read(a.roomId,"human-b")).toBeUndefined();const stores=await Promise.all([a,b].map((room)=>SqliteRoomRepository.open(value.root,value.databasePath,{roomId:room.roomId})));
    ...
    const alphaStore=requiredAt(stores,0,"alpha room store"),betaStore=requiredAt(stores,1,"beta room store");
    await Promise.all([alphaStore.addMessage("you","alpha-only","chat",undefined,undefined,{id:"human-a",name:"Ada"}),betaStore.addMessage("you","beta-only","chat",undefined,undefined,{id:"human-a",name:"Ada"})]);await alphaStore.setSession("codex-sol","session-alpha","read-only");await betaStore.setSession("codex-sol","session-beta","read-only");
    const task=createTask({roomId:a.roomId,taskId:"task-alpha",title:"Only alpha",actor:{id:"human-a",roomRole:"owner"},now:new Date().toISOString()});expect((await alphaStore.createTask(task)).kind).toBe("created");expect(alphaStore.snapshot().messages.map((message)=>message.text)).toContain("alpha-only");expect(alphaStore.snapshot().messages.map((message)=>message.text)).not.toContain("beta-only");expect(betaStore.snapshot().sessions["codex-sol"]?.id).toBe("session-beta");expect((await betaStore.listTasks({roomId:b.roomId})).items).toEqual([]);stores.forEach((store)=>store.close());value.lifecycle.close();
  });
