...
export class InvestigationStore {
  ...
  async complete(expectedRevision: number, record: InvestigationRecord, entry: InvestigationInboxEntry, event: InvestigationEvent, maxEntries: number) { return this.mutate((state) => { const before = state.jobs[record.investigationId]; if (!before || before.revision !== expectedRevision || state.inbox[entry.inboxEntryId]) return { result: false }; const inbox = { ...state.inbox, [entry.inboxEntryId]: entry }; const active = Object.values(inbox).filter((item) => item.owner === entry.owner && (item.status === "UNREAD" || item.status === "ACKNOWLEDGED")).sort((a, b) => a.createdAt.localeCompare(b.createdAt)); for (const stale of active.slice(0, Math.max(0, active.length - maxEntries))) inbox[stale.inboxEntryId] = { ...stale, revision: stale.revision + 1, status: "ARCHIVED", updatedAt: entry.createdAt }; return { state: { ...state, jobs: { ...state.jobs, [record.investigationId]: record }, inbox, events: [...state.events, event] }, result: true }; }); }
