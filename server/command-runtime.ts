...
import { MAX_COMMAND_DELIVERY_MESSAGE, MAX_DIAGNOSTIC_PROMPT_HEAD, MAX_DIAGNOSTIC_TEXT, publicPollProjection, type CommandAttempt, type CommandDeliveryResult, type CommandGhExecution, type CommandInvoker, type CommandPoll, type CommandPovExecution, type CommandRecordStore, type CommandSubmission, type DiagnosticRecord, type GhProjection, type PublicPollProjection } from "./command-record.js";
import type { GitHubReadCommandService } from "./github-read-service.js";
...
export interface CommandRuntimeDependencies {
  ...
  readonly githubRead?: GitHubReadCommandService;
  readonly publishGhResult?: (executionId:string,summary:string,text:string)=>Promise<void>;
...
function generationField(generationId:string|null|undefined){return generationId?{generationId}:{};}
function recoveredExecutionResult(generationId:string|null|undefined,visibleMessages:readonly string[],deliveryResult:CommandDeliveryResult|undefined):CommandExecutionResult{return{...generationField(generationId),visibleMessages,...deliveryResult};}
function advancePovExecution(execution:CommandPovExecution,agentId:ActiveAgentId,updatedAt:string):CommandPovExecution{return{executionId:execution.executionId,roomId:execution.roomId,submissionId:execution.submissionId,targetAgentIds:execution.targetAgentIds,processedTargetAgentIds:[...execution.processedTargetAgentIds,agentId],currentTargetAgentId:null,generationId:null,status:execution.status,reason:execution.reason,createdAt:execution.createdAt,updatedAt};}
...
export class CommandRuntime {
  ...
  async initialize() {
    ...
    for(const execution of await this.dependencies.store.listPendingPovExecutions(this.roomId)){const submission=await this.dependencies.store.getCommandSubmission(this.roomId,execution.submissionId);if(submission?.invocation.command==="pov")await this.startPov(execution,submission);}
  ...
  async close() {
    ...
    // Provider cancellation must not wait forever, but already-started durable
    // writes must finish before the caller closes or removes the backing store.
  ...
  async submit(input: CommandInput, invoker: CommandInvoker, clientSubmissionId: string): Promise<CommandResponse> {
    if (this.closing) return { kind: "private-error", message: "The command runtime is shutting down." };
  ...
  private async replay(submission: CommandSubmission): Promise<CommandResponse> {
    ...
    if(submission.command==="gh"){const execution=await this.dependencies.store.getGhExecution(this.roomId,submission.submissionId);if(!execution)return{kind:"private-error",message:"The original GitHub command is still being recovered."};if(execution.status==="queued")return this.executeGh(execution,submission,true);try{await this.dependencies.githubRead?.validateLease(this.roomId,execution.authorizationLease);}catch(error){const failure=this.dependencies.githubRead?.failure(error);return{kind:"private-error",message:failure?.text||"GitHub reads are not available for the current room binding."};}await this.deliverGhResult(execution,submission).catch(()=>undefined);return{kind:"accepted",submissionId:submission.submissionId,duplicate:true,...(execution.projection?{github:execution.projection}:{}),...(execution.renderedText?{resultText:execution.renderedText}:{})};}
  ...
  private async dispatch(submission: CommandSubmission): Promise<CommandResponse> {
    ...
    if(invocation.command==="gh"){
      ...
      let authorizationLease:string;try{authorizationLease=await this.dependencies.githubRead.authorize(this.roomId);}catch(error){const failed=this.dependencies.githubRead.failure(error);await this.dependencies.capabilityAudit?.({agentId:submission.invoker.id,capability:"github_read",outcome:"denied",correlationId:submission.submissionId,reason:failed.kind});return{kind:"private-error",message:failed.text};}
  ...
  private async executeGh(execution:CommandGhExecution,submission:CommandSubmission,duplicate=false):Promise<CommandResponse>{
    ...
    let current=await this.dependencies.store.getGhExecution(this.roomId,submission.submissionId);if(!current)return{kind:"private-error",message:"GitHub execution metadata is unavailable."};if(current.status!=="queued"){try{await this.dependencies.githubRead.validateLease(this.roomId,current.authorizationLease);}catch(error){return{kind:"private-error",message:this.dependencies.githubRead.failure(error).text};}await this.deliverGhResult(current,submission).catch(()=>undefined);return{kind:"accepted",submissionId:submission.submissionId,duplicate:true,...(current.projection?{github:current.projection}:{}),...(current.renderedText?{resultText:current.renderedText}:{})};}
    if(!current.authorizationLease){let authorizationLease:string;try{authorizationLease=await this.dependencies.githubRead.authorize(this.roomId);}catch(error){const failed=this.dependencies.githubRead.failure(error);return this.terminalizeGh(current,submission,failed.kind,failed.text);}const adopted=await this.dependencies.store.adoptGhAuthorizationLease(current.roomId,current.executionId,current.updatedAt,authorizationLease,timestamp(this.clock,current.updatedAt));current=adopted.kind==="accepted"?adopted.execution:(await this.dependencies.store.getGhExecution(this.roomId,submission.submissionId))!;if(!current||current.status!=="queued"||!current.authorizationLease)return{kind:"private-error",message:"GitHub authorization changed concurrently; retry with the same request ID."};}
    let terminal:CommandGhExecution;
    try{const result=await this.dependencies.githubRead.execute(this.roomId,(submission.invocation as Extract<CommandInvocation,{command:"gh"}>).selector,current.authorizationLease);await this.dependencies.githubRead.validateLease(this.roomId,result.authorizationLease);terminal={...current,authorizationLease:result.authorizationLease,status:"completed",deliveryStatus:"pending",projection:result.projection,renderedText:result.renderedText,failureKind:null,diagnostics:result.diagnostics,updatedAt:timestamp(this.clock,current.updatedAt)};await this.dependencies.capabilityAudit?.({agentId:submission.invoker.id,capability:"github_read",outcome:"completed",correlationId:submission.submissionId});}
    ...
    const saved=await this.dependencies.store.compareAndSetGhExecution(current.updatedAt,terminal);let durable=saved.kind==="accepted"?saved.execution:await this.dependencies.store.getGhExecution(this.roomId,submission.submissionId);if(!durable||durable.status==="queued")return{kind:"private-error",message:"GitHub execution changed concurrently; retry with the same request ID."};try{await this.dependencies.githubRead.validateLease(this.roomId,durable.authorizationLease);}catch(error){if(durable.deliveryStatus==="pending"){const dropped=await this.dependencies.store.markGhExecutionDelivered(durable.roomId,durable.executionId,durable.updatedAt,timestamp(this.clock,durable.updatedAt));if(dropped.kind==="accepted")durable=dropped.execution;}return{kind:"private-error",message:this.dependencies.githubRead.failure(error).text};}await this.deliverGhResult(durable,submission).catch(()=>undefined);return{kind:"accepted",submissionId:submission.submissionId,duplicate,...(durable.projection?{github:durable.projection}:{}),...(durable.renderedText?{resultText:durable.renderedText}:{})};
  }

  private async deliverGhResult(execution:CommandGhExecution,knownSubmission?:CommandSubmission){if(execution.status==="queued"||execution.deliveryStatus==="delivered"||!execution.renderedText||!this.dependencies.publishGhResult)return;const submission=knownSubmission||await this.dependencies.store.getCommandSubmission(this.roomId,execution.submissionId);if(!submission||!this.authorized(submission))return;try{await this.dependencies.githubRead?.validateLease(this.roomId,execution.authorizationLease);}catch{return;}await this.dependencies.publishGhResult(execution.executionId,this.githubAuditText(submission),execution.renderedText);await this.dependencies.githubRead?.validateLease(this.roomId,execution.authorizationLease);await this.dependencies.store.markGhExecutionDelivered(execution.roomId,execution.executionId,execution.updatedAt,timestamp(this.clock,execution.updatedAt));}
  ...
  private githubAuditText(submission: CommandSubmission) {
    return `— ${safeLabel(submission.invoker.displayName)} ran /gh — Read-only repository query`;
  }
  ...
  private observe<T>(operation: Promise<T>): Promise<T> {
    ...
    void operation.then(() => this.backgroundOperations.delete(operation), () => this.backgroundOperations.delete(operation));
    return operation;
  }

  private background(operation: Promise<unknown>) {
    this.observe(operation.catch((error) => logOperationSafely(this.dependencies.operationLog, "error", "command.background.failed", { error, outcome: "failed", reason: "durable-recovery-will-retry" })));
  }

  private async providerResult(operation: Promise<CommandExecutionResult>) {
    const signal = this.shutdown.signal;
    let cancelled!: () => void;
    const cancellation = new Promise<never>((_resolve, reject) => {
      cancelled = () => reject(new Error("Command provider wait cancelled by shutdown."));
  ...
  private async launch(submission: CommandSubmission, agentId: ActiveAgentId, attemptNumber: number, acceptedAttempt?:CommandAttempt, recovered=false) {
    if (this.closing || attemptNumber > MAX_COMMAND_ATTEMPTS) return;
    ...
    live.timer = this.clock.setTimeout(()=>this.background(this.stage1(attempt,submission)),this.stage1Ms);
    this.background(this.providerResult(this.dependencies.executeTask(agentId,(submission.invocation as Extract<CommandInvocation,{command:"task"}>).prompt,{
      signal:live.controller.signal,
      partial:(text)=>{ if(this.closing)return;live.partial=sanitizeDiagnosticText(text)||"";this.background(this.captureLateStallPartial(attempt,submission,live.partial)); },
      active:(generationId)=>this.closing?Promise.resolve(false):this.observe(this.markActive(attempt,generationId,submission)),
    })).then((result)=>{if(!this.closing)return this.complete(attempt,submission,result);}).catch((error)=>{if(!this.closing)return this.fail(attempt,submission,error);}));
  }
  ...
  private async markActive(attempt: CommandAttempt,generationId:string,submission:CommandSubmission) {
    ...
    if(claimed.kind!=="accepted" || this.closing) return false;
    if (live.timer) this.clock.clearTimeout(live.timer);
    if(live.reservation?.activate)live.reservation.activate(generationId);else live.reservation?.release();
    live.timer=this.clock.setTimeout(()=>this.background(this.stage2(active,submission)),this.stage2Ms);
    return true;
  }
  ...
  private async complete(attempt:CommandAttempt,submission:CommandSubmission,result:CommandExecutionResult) {
    ...
    const claimed=await this.dependencies.store.compareAndSetCommandAttempt(current.updatedAt,pending);if(claimed.kind!=="accepted")return;this.clearLive(attempt.attemptId);await this.resumeDelivery(pending,submission,result);if(!visible.length){const diagnosticText=result.diagnosticText||result.rawText;await this.captureDiagnostic({agentId:current.agentId,attemptId:current.attemptId,...generationField(pending.generationId),correlationId:`${current.attemptId}:no-response`,prompt:(submission.invocation as Extract<CommandInvocation,{command:"task"}>).prompt,reason:"no-response-needed",...(diagnosticText?{text:diagnosticText}:{}),metadata:{visibleMessages:0}});}
  }

  private async resumeDelivery(attempt:CommandAttempt,submission:CommandSubmission,result?:CommandExecutionResult){if(result)authorizeSourceWorkForCurrentBoot(this.dependencies.store,"command-delivery",attempt.attemptId);const messages=attempt.deliveryMessages||[];const durableResult=result||recoveredExecutionResult(attempt.generationId,messages,attempt.deliveryResult);const provenanceBlocker=durableResult.permission==="writable"?await sourceWorkReconciliationBlocker(this.dependencies.store,"command-delivery",attempt.attemptId):null;if(provenanceBlocker||!this.attemptCurrent(attempt)){await this.captureDiagnostic({agentId:attempt.agentId,attemptId:attempt.attemptId,...generationField(attempt.generationId),correlationId:`${attempt.attemptId}:authority-changed`,prompt:(submission.invocation as Extract<CommandInvocation,{command:"task"}>).prompt,reason:provenanceBlocker?"provenance-reconciliation-required":"authority-changed-before-delivery",text:messages.join("\n"),metadata:{visibleMessages:messages.length}});const dropped={...attempt,status:"completed" as const,reason:provenanceBlocker||attempt.reason,updatedAt:timestamp(this.clock,attempt.updatedAt)};await this.dependencies.store.compareAndSetCommandAttempt(attempt.updatedAt,dropped);return;}await this.dependencies.deliverTask(attempt.attemptId,attempt.agentId,messages,durableResult);const completed={...attempt,status:"completed" as const,updatedAt:timestamp(this.clock,attempt.updatedAt)};await this.dependencies.store.compareAndSetCommandAttempt(attempt.updatedAt,completed);}
