...
export class CommandRuntime {
  ...
  async initialize() {
    ...
    for (const attempt of await this.dependencies.store.listPendingCommandAttempts(this.roomId)) {
      ...
      else if(attempt.status==="queued")await this.launch(submission,attempt.agentId,attempt.attempt,attempt);
  ...
  private async runPovTargets(execution:CommandPovExecution,submission:CommandSubmission,controller:AbortController){for(;;){if(controller.signal.aborted)throw new Error("POV execution was cancelled.");const current=await this.dependencies.store.getPovExecution(this.roomId,submission.submissionId);if(!current||current.status!=="active")return;if(current.currentTargetAgentId){await this.resumePovDelivery(current);continue;}const agentId=current.targetAgentIds.find((candidate)=>!current.processedTargetAgentIds.includes(candidate));if(!agentId)return;const result=await this.dependencies.executePov(agentId,(submission.invocation as Extract<CommandInvocation,{command:"pov"}>).prompt,controller.signal);const messages=(result.visibleMessages||[]).filter(Boolean).slice(0,3).map((message)=>message.slice(0,MAX_COMMAND_DELIVERY_MESSAGE));const outbox={...current,currentTargetAgentId:agentId,generationId:result.generationId||null,deliveryMessages:messages,deliveryResult:durableDeliveryResult(result),updatedAt:timestamp(this.clock,current.updatedAt)};const claimed=await this.dependencies.store.compareAndSetPovExecution(current.updatedAt,outbox);if(claimed.kind!=="accepted")throw new Error("POV target ownership changed before durable result persistence.");await this.resumePovDelivery(outbox,result);}}
  private async resumePovDelivery(execution:CommandPovExecution,result?:CommandExecutionResult){const agentId=execution.currentTargetAgentId;if(!agentId)return;const messages=execution.deliveryMessages||[];try{await this.dependencies.deliverPov(stableId(execution.executionId,agentId),agentId,messages,result||{generationId:execution.generationId||undefined,visibleMessages:messages,...execution.deliveryResult});}catch(error){throw new PovDeliveryPendingError(error instanceof Error?error.message:String(error));}const completed={...execution,processedTargetAgentIds:[...execution.processedTargetAgentIds,agentId],currentTargetAgentId:null,generationId:null,deliveryMessages:undefined,deliveryResult:undefined,updatedAt:timestamp(this.clock,execution.updatedAt)};const claimed=await this.dependencies.store.compareAndSetPovExecution(execution.updatedAt,completed);if(claimed.kind!=="accepted")throw new Error("POV delivery completion changed concurrently.");}
