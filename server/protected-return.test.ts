...
describe("protected return contract", () => {
  it("validates relevance and bounded visible output, permitting an explicit no-update", () => {
    ...
    for (const value of ['{"relevance":"superseded","text":"obsolete finding"}', 'plain prose', '{"relevance":"unknown","text":"text"}', '{"relevance":"relevant","text":"","extra":true}', JSON.stringify({ relevance: "relevant", text: "a".repeat(4_001) })]) expect(() => parseProtectedReturn(value, "cursor")).toThrow();
  });
