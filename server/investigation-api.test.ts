...
describe("investigation API boundary", () => {
  it("does not expose raw context, project identity, capabilities, checkpoint state, or provider session", () => {
    ...
    const protectedJob = projectInvestigation({ ...job, investigationId: "protected-fixture", evidenceRefs: [{ kind: "project_artifact", ref: "unassessed-evidence" }], resultSummary: "unassessed-result", checkpoint: { ...job.checkpoint!, summary: "unassessed-checkpoint" }, unresolvedQuestions: ["unassessed-question"], blocker: "unassessed-progress" });
    expect(JSON.stringify(protectedJob)).not.toContain("unassessed");
    expect(protectedJob).toMatchObject({ investigationId: "protected-fixture", status: "CHECKPOINTED", checkpoint: null, resultSummary: null, evidenceRefs: [], unresolvedQuestions: [] });
  ...
  it("omits protected inbox contents and audit detail from authenticated endpoints", async () => {
    const acknowledgePublicInbox = vi.fn(async () => ({ kind: "not_found" }));
    const service = { inbox: async () => [{ investigationId: "protected-fixture", summary: "unassessed-result" }, { investigationId: "ordinary", summary: "Visible autonomous finding" }], audit: async () => [{ eventId: "event", investigationId: "protected-fixture", revision: 1, at: "2026-09-09T00:00:00Z", action: "WAITING_TOOL", fromStatus: "RUNNING", toStatus: "WAITING_TOOL", usage: {}, detail: "unassessed-progress", projectionHash: "private-hash" }], acknowledgePublicInbox } as unknown as InvestigationService;
    const app = express(); app.use(express.json());
    registerInvestigationRoutes({ app, service, humans: { get: () => ({ id: "fixture-human" }) } as unknown as HumanPresenceRegistry, sessions: { humanId: () => "fixture-human" } as unknown as HumanSessions, broadcast() {} });
    const server = app.listen(0, "127.0.0.1"); await new Promise<void>((resolve) => server.once("listening", resolve));
    const base = `http://127.0.0.1:${(server.address() as AddressInfo).port}`;
    try {
      expect(await (await fetch(base + "/api/investigations/inbox/codex-sol")).json()).toEqual([{ investigationId: "ordinary", summary: "Visible autonomous finding" }]);
      const audit = await (await fetch(base + "/api/investigations/protected-fixture/audit")).json();
      expect(JSON.stringify(audit)).not.toMatch(/unassessed|private-hash/); expect(audit[0].action).toBe("WAITING_TOOL");
      const response = await fetch(base + "/api/investigations/inbox/protected-entry/acknowledge", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ close: true }) });
      expect(response.status).toBe(404); expect(acknowledgePublicInbox).toHaveBeenCalledWith("protected-entry", true);
