...
import {
  CONTINUATION_POLICY_VERSION, canTransitionContinuation, continuationIsNonterminal, projectPathHash, redactContinuationText,
...
export class ContinuationService {
  ...
  private async failValidation(record: ContinuationRecord, reason: string): Promise<ContinuationResult<ContinuationRecord>> {
    ...
    const now = this.now(); const next = transition(current, "CANCELLED", now, { blocker: reason, cancellationRequested: true, resultDisposition: "CLOSED", completedAt: now });
