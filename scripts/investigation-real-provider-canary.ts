...
async function journal() { try { const directory = path.join(dataDirectory, "logs", "authoritative-v1"); const files = (await readdir(directory)).filter((name) => name.startsWith("generations.") && name.endsWith(".jsonl")); return (await Promise.all(files.map((name) => readFile(path.join(directory, name), "utf8")))).join("").trim().split("\n").filter(Boolean).map((line) => JSON.parse(line)); } catch { return []; } }
