...
import { captureSchema, hashBytes, validateVisualReview, VISUAL_QUESTIONS } from "./visual-review.js";
...
function fixture() {
  ...
  const review = { schemaVersion: 1, inputDigest: digest, reviews: captures.map((capture) => ({ key: capture.key, screenshotSha256: capture.screenshotSha256, reviewerAgentId: "independent-reviewer", reviewedAt: "2026-08-30T01:00:00.000Z", inspectedImage: true, answers: Object.fromEntries(VISUAL_QUESTIONS.map((question) => [question, { verdict: "pass", observation: `Observed ${question}: the screenshot shows the expected composition.` }])) })) };
  return { run, review, digest, readImage: () => bytes };
}
