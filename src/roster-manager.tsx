...
export function RosterManagerDialog({ initialRoster, initialSelectedAgentId, agentListSort = "room", onAgentListSortChange, returnFocusTo, onSaved, onClose, onOpenAdministration, onOpenOpenRouterAccount }: {
  ...
  // Own their native overflow decoration directly so WebKit overlay scrollbars do
  // not conceal reachable roster rows or form fields.
  useScrollEdges(rosterEditorRef, `${loading}:${displayedEntries.length}:${mobilePane}`);
  useScrollEdges(detailPaneRef, `${selectedAgentId || "model-picker"}:${changingModelForAgentId || ""}:${mobilePane}`);
                  ...
                  <section className="roster-discovery roster-discovery--workspace" aria-labelledby="roster-explore-heading">
                    {newSelectedModel ? (
                      <>
                        ...
                        <form className="roster-create-panel" onSubmit={(event) => { event.preventDefault(); if (newName.trim() && !saving) createParticipant(); }}><h4>Name and configure the agent</h4><label>Agent alias<input ref={newNameInputRef} value={newName} maxLength={48} aria-invalid={Boolean(newNameError)} aria-describedby={newNameError ? "new-agent-alias-error" : undefined} placeholder="For example: Scout or Code Coach" onChange={(event) => { setNewName(event.target.value); setNewNameError(""); }} />{newNameError ? <small id="new-agent-alias-error" className="roster-field-error" role="alert">{newNameError}</small> : null}</label><div className="roster-model-options"><label>Variant / reasoning effort<select className="classic-select" value={newVariant} onChange={(event) => setNewVariant(event.target.value)}><option value="">Default</option>{newSelectedModel.variants?.map(({ id, displayName }) => <option value={id} key={id}>{displayName}{newSelectedModel.capabilities?.reasoningEffort?.includes(id) ? " (reasoning effort)" : ""}</option>)}</select></label></div><div className="roster-create-actions"><small>Next, review the agent’s settings before saving the roster.</small><button type="submit" className="classic-button roster-create-button" data-default-button disabled={!newName.trim() || saving}>Review agent</button></div></form>
                      </>
                    ) : (
                      <>
