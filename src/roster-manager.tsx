          ...
          {loading ? <p role="status">Loading roster…</p> : (
            <div className="roster-editor" role="list" aria-label="Room agent roster">
              {entries.map((entry, index) => {
                ...
                const selectedReference = { ...(entry.providerId ? { providerId: entry.providerId } : {}), modelId: entry.modelId || profile?.modelId || "", ...(entry.variant ? { variant: entry.variant } : {}), ...(entry.reasoningEffort ? { reasoningEffort: entry.reasoningEffort } : {}) };
                const selectedModel = discoveredModels.find((model) => model.modelId === selectedReference.modelId && (model.providerId || "") === (selectedReference.providerId || ""));
                ...
                return (
                  <div className="roster-editor-row" role="listitem" key={entry.agentId}>
                    ...
                    <details className="roster-config-editor">
                      ...
                      <label>Model<select value={`${entry.providerId || ""}\u0000${entry.modelId || profile?.modelId || ""}`} onChange={(event) => { const [providerId, modelId] = event.target.value.split("\u0000"); const discovered = discoveredModels.some((model) => model.modelId === modelId && (model.providerId || "") === providerId); replaceAt(index, { ...entry, providerId: providerId || undefined, modelId, variant: undefined, reasoningEffort: undefined, ...(discovered ? { sessionInvalidationReason: undefined, selectionConfirmationRequired: undefined } : {}) }); }}>{!selectedModel ? <option value={`${entry.providerId || ""}\u0000${entry.modelId || profile?.modelId || ""}`}>{entry.providerId ? `${entry.providerId}/` : ""}{entry.modelId || profile?.modelId}{selectedModelAvailable ? "" : " (currently unavailable)"}</option> : null}{discoveredModels.map((model) => <option key={`${model.providerId || ""}/${model.modelId}`} value={`${model.providerId || ""}\u0000${model.modelId}`}>{model.displayName}</option>)}</select></label>
                      ...
                      {entry.sessionInvalidationReason ? <div className="roster-diagnostic"><p>{entry.sessionInvalidationReason}</p>{entry.selectionConfirmationRequired && selectedModelAvailable ? <button type="button" className="classic-button" onClick={() => replaceAt(index, { ...entry, sessionInvalidationReason: "", selectionConfirmationRequired: undefined })}>Confirm selected OpenCode model</button> : null}</div> : null}
                    </details>
                  </div>
                );
              })}
