...
export function Continuations({ refreshKey }: { refreshKey: number }) {
  ...
  const refresh = useCallback(async () => { try { const next = await loadContinuations(); setData(next); const owners = [...new Set(next.jobs.map((job) => job.owner))]; setInbox(Object.fromEntries(await Promise.all(owners.map(async (owner) => [owner, await loadContinuationInbox(owner)])))); setError(""); } catch (failure) { setError(failure instanceof Error ? failure.message : "Could not load continuations."); } }, []);
  useEffect(() => { void refresh(); const timer = window.setInterval(() => void refresh(), 2_000); return () => window.clearInterval(timer); }, [refresh, refreshKey]);
  ...
  return <section className="tasks-workspace continuation-workspace" aria-label="Durable continuations">
    <header className="tasks-header"><div><h2>Durable Continuations</h2><p>Bounded assignment work stays out of the room transcript until you explicitly use its result.</p></div>{data?.policy ? <label><input type="checkbox" checked={data.policy.enabled} onChange={(event) => void mutate(() => setContinuationPolicy(data.policy.revision, event.target.checked))} /> Initiative enabled</label> : null}</header>
    {error ? <div className="error-strip" role="alert">{error}</div> : null}
    {!data?.jobs.length ? <p className="task-empty">No continuation jobs.</p> : data.jobs.map((job) => <article className="task-card" key={job.jobId}>
      ...
      {job.blocker ? <p role="status"><strong>Blocked:</strong> {job.blocker}</p> : null}
