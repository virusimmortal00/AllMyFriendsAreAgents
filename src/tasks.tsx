...
export function Tasks({ refreshKey = 0 }: { refreshKey?: number }) {
  ...
  async function startContinuation(assignmentReferenceId: string, objective: string) {
    if (!detail) return; setBusy(true); setError(""); setNotice("");
    try {
  ...
  return <section className="tasks-panel beveled-inset" aria-label="Room tasks">
    ...
    <div className="tasks-body">
      ...
      </> : detail ? <TaskDetail detail={detail} busy={busy} editTitle={editTitle} editDescription={editDescription} setEditTitle={(value) => { titleDirty.current = true; setEditTitle(value); }} setEditDescription={(value) => { descriptionDirty.current = true; setEditDescription(value); }} saveField={saveField} mutate={mutate} startContinuation={startContinuation} /> : <div className="task-empty"><strong>Task unavailable.</strong><button type="button" className="classic-button" onClick={() => setSelected(null)}>Return to task list</button></div>}
    </div>
  </section>;
}

function TaskDetail({ detail, busy, editTitle, editDescription, setEditTitle, setEditDescription, saveField, mutate, startContinuation }: {
  detail: TaskDetailResponse; busy: boolean; editTitle: string; editDescription: string; setEditTitle: (value: string) => void; setEditDescription: (value: string) => void; saveField: (field: "title" | "description") => void; mutate: (action: string, body?: Record<string, unknown>) => void; startContinuation: (assignmentReferenceId: string, objective: string) => void;
}) {
  ...
  const [continuationObjectives, setContinuationObjectives] = useState<Record<string, string>>({});
  ...
  return <article className="task-detail" aria-labelledby="task-title">
    ...
    <div className="task-columns">
      ...
      <section><h3>Assignment links</h3><Lines values={task.references.filter((item) => item.kind === "assignment").map((item) => `${item.targetId} — ${item.completionState}`)} empty="No assignments linked." />{["approved", "active", "blocked"].includes(task.state) ? task.references.filter((item) => item.kind === "assignment" && item.completionState !== "finished").map((item) => <form key={`continuation:${item.id}`} onSubmit={(event) => { event.preventDefault(); const objective = continuationObjectives[item.id]?.trim(); if (objective) startContinuation(item.id, objective); }}><textarea aria-label={`Continuation objective for ${item.targetId}`} maxLength={4000} value={continuationObjectives[item.id] || ""} onChange={(event) => setContinuationObjectives((current) => ({ ...current, [item.id]: event.target.value }))} placeholder={`Bounded background objective for ${item.targetId}`} /><button className="classic-button" disabled={busy || !continuationObjectives[item.id]?.trim()}>Start continuation</button></form>) : null}<form onSubmit={(event) => { event.preventDefault(); if (assignmentId) mutate("assign", { assignmentId }); }}><input aria-label="Existing assignment ID" value={assignmentId} onChange={(event) => setAssignmentId(event.target.value)} placeholder="Existing assignment ID" /><button className="classic-button" disabled={busy}>Link assignment</button></form></section>
