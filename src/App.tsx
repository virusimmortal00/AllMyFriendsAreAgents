...
import { adjacentTranscriptMagnification, loadTranscriptMagnification, loadTranscriptMessagePrices, loadTranscriptSystemActivity, loadTranscriptTimestamps, saveTranscriptMagnification, saveTranscriptMessagePrices, saveTranscriptSystemActivity, saveTranscriptTimestamps } from "./transcript-view";
...
export default function App() {
  ...
  const [showSystemActivity, setShowSystemActivity] = useState(loadTranscriptSystemActivity);
  ...
  function toggleTranscriptSystemActivity() {
    setShowSystemActivity((current) => {
      const next = !current;
      saveTranscriptSystemActivity(next);
      return next;
    });
  }
  ...
  const menus: ClassicMenuDefinition[] = [
    ...
    defineViewMenu([
        ...
        presentationCommand({ label: "System activity", accessKey: "Y", checked: showSystemActivity, checkType: "checkbox", onSelect: toggleTranscriptSystemActivity }),
        { type: "separator" },
      ...
      <section className="app-window" aria-label="AllMyFriendsAreAgents application">
        ...
        <div className="workspace" data-primary-workspace tabIndex={-1}>
          <section className="chat-panel beveled-inset" {...viewAttributes(VIEWS.roomChat)} data-responsive-view-id={VIEWS.compactRoomChat.id} data-responsive-view-name={VIEWS.compactRoomChat.name} data-responsive-view-state={VIEWS.compactRoomChat.state}>
            <Transcript messages={room.messages} magnification={transcriptMagnification} showTimestamps={showTimestamps} showMessagePrices={showMessagePrices} showSystemActivity={showSystemActivity} transcriptRef={transcript} onOpenImprovement={openImprovement} />
